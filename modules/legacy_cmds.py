...
class Owner(commands.Cog):
    ...
    extra_dirs = ["local_database", ".player_sessions"]
    ...
    def format_log(self, data: list):
        return "\n".join(
            f"[`{c['abbreviated_commit']}`]({self.bot.pool.remote_git_url}/commit/{c['commit']}) `- "
    ...
    @commands.command(
        hidden=True,
        aliases=["gls", "lavalink", "lllist", "lavalinkservers"],
    ...
    async def getlavaservers(self, ctx: CustomContext):
        ...
        await ctx.send(
            embed=disnake.Embed(
                ...
                "I will need to restart to use the servers in this file.**"
            )
        )

    updatelavalink_flags = CommandArgparse()
    updatelavalink_flags.add_argument(
        "-yml",
        "--yml",
        action="store_true",
    ...
    async def restartlavalink(self, ctx: CustomContext):
        ...
        await ctx.send(
            embed=disnake.Embed(
                description="**Restarting LOCAL lavalink server.**",
    ...
    @commands.command(
        hidden=True,
        aliases=["ull", "updatell", "llupdate", "llu"],
        extras={"flags": updatelavalink_flags},
    )
    ...
    @panel_command(
        aliases=["rcfg"],
        description="Recarregar as configs do bot.",
        emoji="⚙",
    ...
    @commands.is_owner()
    @panel_command(
        aliases=["rd", "recarregar"],
        description="Recarregar os módulos.",
        emoji="🔄",
        alt_name="Carregar/Recarregar módulos.",
    )
    async def reload(
        self, ctx: Union[CustomContext, disnake.MessageInteraction], *modules
    ):
    ...
    @commands.max_concurrency(1, commands.BucketType.default)
    @panel_command(
        aliases=["up", "atualizar"],
        description="Atualizar meu code usando o git.",
        emoji="<:git:944873798166020116>",
        alt_name="Atualizar Bot",
        extras={"flags": update_flags},
    )
    async def update(
        self, ctx: Union[CustomContext, disnake.MessageInteraction], *, opts: str = ""
    ):  # TODO: Rever se há alguma forma de usar commands.Flag sem um argumento obrigatório, ex: --pip.
        ...
        args, unknown = (
            self.bot.get_command("update")
            .extras["flags"]
            .parse_known_args(opts.split())
        )
        ...
        data = (
            await run_command(f"git --work-tree=. log {commit} {self.git_format}")
        ).split("\n")
        ...
        if isinstance(ctx, CustomContext):
            ...
            self.bot.loop.create_task(
                self.update_deps(ctx, requirements_old, args, use_poetry=use_poetry)
            )
        ...
        if args.pip:
            ...
            if original_reqs != requirements_new:
                ...
                try:
                    prefix = (
                        ctx.prefix
                        if (not str(ctx.guild.me.id) in ctx.prefix)
                        else f"@{ctx.guild.me.name}"
                    )
                ...
                await ctx.send(
                    ...
                    components=[
                        disnake.ui.Button(
                            ...
                            custom_id="updatecmd_requirements",
                        ),
                        disnake.ui.Button(
                            label="Atualizar dependências",
                            custom_id="updatecmd_installdeps_"
                            + ("poetry" if use_poetry else "pip"),
                        ),
                        disnake.ui.Button(
                            label="Atualizar dependências (force)",
                            custom_id="updatecmd_installdeps_force_"
                            + ("poetry" if use_poetry else "pip"),
                        ),
                    ],
                )
    ...
    async def update_buttons(self, inter: disnake.MessageInteraction):
        ...
        args, unknown = (
            self.bot.get_command("update").extras["flags"].parse_known_args(["-pip"])
        )

        await self.update_deps(
            inter, "", args, use_poetry=inter.data.custom_id.endswith("_poetry")
        )
    ...
    @commands.cooldown(1, 10, commands.BucketType.user)
    @panel_command(
        aliases=["latest", "lastupdate"],
        description="Ver minhas atualizações mais recentes.",
        emoji="📈",
        alt_name="Ultimas atualizações",
        hidden=False,
    )
    async def updatelog(
        self, ctx: Union[CustomContext, disnake.MessageInteraction], amount: int = 10
    ):

        if not os.path.isdir(os.environ["GIT_DIR"]):
            raise GenericError(
                "Não há repositorio iniciado no diretório do bot...\nNota: Use o comando update."
            )
        ...
        txt = (
            f"🔰 ** | [Atualizações recentes:]({self.bot.pool.remote_git_url}/commits/main)**\n\n"
    ...
    async def panel(self, ctx: CustomContext):
        ...
        embed = disnake.Embed(
            title="PAINEL DE CONTROLE.", color=self.bot.get_color(ctx.guild.me)
        )
    ...
    async def setprefix(self, ctx: CustomContext, prefix: str):
        ...
        if not prefix or len(prefix) > 5:
            raise GenericError(
                "**O prefixo não pode conter espaços ou ter acima de 5 caracteres.**"
            )

        guild_data = await self.bot.get_global_data(
            ctx.guild.id, db_name=DBModel.guilds
        )

        self.bot.pool.guild_prefix_cache[ctx.guild.id] = prefix
        guild_data["prefix"] = prefix
        await self.bot.update_global_data(
            ctx.guild.id, guild_data, db_name=DBModel.guilds
        )
    ...
    async def setuserprefix(self, ctx: CustomContext, prefix: str):
        ...
        await self.bot.update_global_data(
            ctx.author.id, user_data, db_name=DBModel.users
        )
    ...
    async def setguildprefix(
        self, ctx: CustomContext, server_id: int, prefix: str = None
    ):

        if not 17 < len(str(server_id)) < 24:
            raise GenericError(
                "**A quantidade de caracteres do id do servidor tem que estar entre 18 a 23.**"
            )
        ...
        if not prefix:
            guild_data["prefix"] = ""
            await ctx.bot.update_global_data(
                server_id, guild_data, db_name=DBModel.guilds
            )
    ...
    @commands.is_owner()
    @panel_command(
        aliases=["expsource", "export", "exs"],
        description="Exportar minha source para um arquivo zip.",
        emoji="💾",
        alt_name="Exportar source/código-fonte.",
    )
    async def exportsource(
        self, ctx: Union[CustomContext, disnake.MessageInteraction], *, flags: str = ""
    ):
        ...
        for env, value in os.environ.items():
            if (e := env.lower()).startswith(
                ("token_bot_", "test_guilds_", "lavalink_node_")
            ) or e == "token":
        ...
        for folder, subfolder, files in os.walk("./modules"):
            for file in files:
                ...
                ) not in filelist.split("\n"):
                    filelist += f"\n{filename}"

        for extra_dir in self.extra_dirs:
            for dir_path, dir_names, filenames in os.walk(extra_dir):
                filelist += "\n" + "\n".join(
                    os.path.join(dir_path, file) for file in filenames
                )
        ...
        if (filesize := (os.path.getsize("source.zip")) / 1024) > 25600:
            try:
                ...
                pass
            raise GenericError(
                f"**O tamanho do arquivo ultrapassou do limite de 25MB (tamanho atual: {humanize.naturalsize(filesize)})**"
            )
        ...
        try:
            ...
            embed.set_footer(
                text="Por medida de segurança, esta mensagem será deletada em 2 minutos."
            )

            msg = await ctx.author.send(
                embed=embed,
                file=disnake.File(
                    "./source.zip", filename=f"{self.bot.user}_source.zip"
                ),
                delete_after=120,
            )
    ...
    async def cleardm(self, ctx: CustomContext, amount: int = 20):
        ...
        if not counter:
            raise GenericError(
                f"**Nenhuma mensagem foi deletada de {amount} verificada{'s'[:amount^1]}...**"
            )
        ...
        await ctx.send(
            embed=disnake.Embed(
                description=txt, colour=self.bot.get_color(ctx.guild.me)
            )
        )
    ...
    async def close_shell_result(self, inter: disnake.MessageInteraction):
        ...
        if not await self.bot.is_owner(inter.author):
            return await inter.send(
                "**Apenas meu dono pode usar este botão!**", ephemeral=True
            )
    ...
    async def shell(self, ctx: CustomContext, *, command: str):
        ...
        try:
            ...
            kwargs = {}
            if len(result) > 2000:
                kwargs["file"] = string_to_file(
                    result, filename=f"shell_result_{ctx.message.id}.txt"
                )
            else:
                kwargs["content"] = f"```py\n{result}```"
            ...
            await ctx.reply(
                components=[
                    disnake.ui.Button(
                        label="Fechar Shell", custom_id="close_shell_result", emoji="♻️"
                    )
                ],
    ...
    @commands.command(
        description="inicializar um player no servidor.",
        aliases=["spawn", "sp", "spw", "smn"],
    )
        ...
        if bot.user.id not in ctx.author.voice.channel.voice_states:
            ...
            if len(free_bots) > 1:
                ...
                if not v.inter.author.voice:
                    await msg.edit(
                        embed=disnake.Embed(
                            description="### Você não está conectado em um voice channel...",
                            color=self.bot.get_color(guild.me),
                        ),
                        view=None,
                    )
    ...
    @commands.command(
        ...
        description="Alterar o banner do bot usando anexo ou link direto de uma imagem jpg ou gif.",
    )
    ...
    @commands.command(
        ...
        description="Alterar o avatar do bot usando anexo ou link direto de uma imagem jpg ou gif.",
    )
    async def setavatar(self, ctx: CustomContext, url: str = "", mode="avatar"):
        ...
        if not url:
            ...
            if not ctx.message.attachments:
                raise GenericError(
                    "Você deve informar o link de uma imagem ou gif (ou anexar uma) no comando."
                )

            url = ctx.message.attachments[0].url

            if not url.split("?ex=")[0].endswith(
                (".png", ".jpg", ".jpeg", ".webp", ".gif", ".bmp")
            ):
                raise GenericError(
                    "Você deve anexar um arquivo válido: png, jpg, jpeg, webp, gif, bmp."
                )
        ...
        await func(
            f"O novo {mode} do bot {bot.user.mention} está sendo processado. Por favor aguarde...",
            embed=None,
            view=None,
        )
        ...
        if mode == "avatar":
            await bot.http.request(
                ...
                json={"icon": disnake.utils._bytes_to_base64_data(image_bytes)},
            )
        ...
        await func(
            f"O {avatar_txt} do bot {bot.user.mention} foi alterado com sucesso.",
            view=None,
            embed=None,
        )
