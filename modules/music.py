...
from utils.music.checks import (
    check_voice,
    has_player,
    has_source,
    is_requester,
...
from utils.music.errors import (
    ...
    DiffVoiceChannel,
    NoPlayer,
    YoutubeSourceDisabled,
)
from utils.music.interactions import (
    VolumeInteraction,
    QueueInteraction,
    SelectInteraction,
    FavMenuView,
...
from utils.others import (
    ...
    string_to_file,
    CommandArgparse,
    music_source_emoji_url,
    song_request_buttons,
    select_bot_pool,
    ProgressBar,
    update_inter,
    get_source_emoji_cfg,
)
...
class Music(commands.Cog):
    ...
    def __init__(self, bot: BotCore):
        ...
        self.song_request_concurrency = commands.MaxConcurrency(
            1, per=commands.BucketType.member, wait=False
        )

        self.player_interaction_concurrency = commands.MaxConcurrency(
            1, per=commands.BucketType.member, wait=False
        )

        self.song_request_cooldown = commands.CooldownMapping.from_cooldown(
            rate=1, per=300, type=commands.BucketType.member
        )

        self.music_settings_cooldown = commands.CooldownMapping.from_cooldown(
            rate=3, per=15, type=commands.BucketType.guild
        )
    ...
    @pool_command(
        ...
        cooldown=stage_cd,
        max_concurrency=stage_mc,
        extras={"exclusive_cooldown": True},
    ...
    @commands.slash_command(
        description=f"{desc_prefix}Activate/editar o sistema de anúncio/status automático do canal com o nome da música.",
    ...
    async def set_voice_status(
        ...
        if not author.guild_permissions.manage_guild and not (
            ...
            raise GenericError(
                "**Você não possui permissão de gerenciar servidor para activate/disable esse sistema.**"
            )

        if not template:
            await inter.response.defer(ephemeral=True, with_message=True)
            global_data = await self.bot.get_global_data(
                inter.guild_id, db_name=DBModel.guilds
            )
            view = SetStageTitle(ctx=inter, bot=bot, data=global_data, guild=guild)
            view.message = await inter.send(
                view=view, embeds=view.build_embeds(), ephemeral=True
            )
            await view.wait()
        else:
            if not any(p in template for p in SetStageTitle.placeholders):
                raise GenericError(
                    f"**Você deve usar pelo menos um placeholder válido:** {SetStageTitle.placeholder_text}"
                )
    ...
    async def message_play(self, inter: disnake.MessageCommandInteraction):
        ...
        if not inter.target.content:
            emb = disnake.Embed(
                description=f"Não há texto na [mensagem]({inter.target.jump_url}) selecionada...",
    ...
    async def search(
        ...
        *,
        position: int = commands.Param(
            name="posição",
            description="Colocar a música em uma posição específica",
            default=0,
        ),
        ...
        options: str = commands.Param(
            name="opções",
            description="Opções para processar playlist",
            choices=playlist_opts,
            default=False,
        ),
        repeat_amount: int = commands.Param(
            name="repetições",
            description="definir quantidade de repetições.",
            default=0,
        ),
        server: str = commands.Param(
            name="server",
            desc="Usar um servidor de música específico na busca.",
            default=None,
        ),
    ...
    async def do_connect(
        ...
        if not channel:
            try:
                channel = (
                    ctx.music_bot.get_channel(ctx.author.voice.channel.id)
                    or ctx.author.voice.channel
                )
            except AttributeError:
        ...
        can_connect(
            channel, me.guild, check_other_bots_in_vc=check_other_bots_in_vc, bot=bot
        )

        deafen_check = True

        if (
            isinstance(ctx, disnake.AppCmdInter)
            and ctx.application_command.name == self.connect.name
        ):
        ...
        if isinstance(channel, disnake.StageChannel):
            ...
            if stage_perms.mute_members:
                ...
                embed.description = (
                    f"**Preciso que algum staff me convide para falar no palco: "
                ...
                await text_channel.send(
                    ctx.author.mention, embed=embed, delete_after=45
                )
    ...
    @pool_command(
        name="addposition",
        description="Add music to a specific line in the queue.",
        aliases=["adp", "addpos"],
        check_player=False,
        cooldown=play_cd,
        max_concurrency=play_mc,
    ...
    stage_flags.add_argument(
        "-position",
        "-pos",
        "-p",
        type=int,
        default=0,
        help="Put the music in a specific line in the line (will be ignored if you use -next etc).\nEx: -p 10",
    )
    stage_flags.add_argument(
        "-next",
        "-proximo",
        action="store_true",
        help="Add the music/playlist at the top of the line (equivalent to: -Pos 1)",
    )
    stage_flags.add_argument(
        "-reverse",
        "-r",
        action="store_true",
        help="Invert the order of added songs (effective only when adding playlist).",
    )
    stage_flags.add_argument(
        "-shuffle",
        "-sl",
        action="store_true",
        help="Mix added songs (effective only when adding playlist).",
    )
    stage_flags.add_argument(
        "-select",
        "-s",
        action="store_true",
        help="Choose the music from the results found.",
    )
    stage_flags.add_argument(
        "-force",
        "-now",
        ...
        "-f",
        action="store_true",
        help="Play the music added immediately (effective only if there is a song today.)",
    )
    stage_flags.add_argument(
        "-loop",
        "-lp",
        type=int,
        default=0,
        help="Define the amount of repetitions of the chosen music.\nEx: -loop 5",
    )
    stage_flags.add_argument(
        "-server",
        "-sv",
        type=str,
        default=None,
        help="Use a specific music server.",
    )
    stage_flags.add_argument(
        "-selectbot",
        "-sb",
        action="store_true",
        help="Select a manually available bot.",
    )
    ...
    @pool_command(
        name="search",
        description="Search for music and choose one between the results to play.",
        aliases=["sc"],
        check_player=False,
        cooldown=play_cd,
        max_concurrency=play_mc,
    ...
    async def play_file(
        ...
        file: disnake.Attachment = commands.Param(
            name="file",
            description="Audio file to play or add in line",
        ),
        position: int = commands.Param(
            name="position",
            description="Put the music in a specific position",
            default=0,
        ),
        force_play: str = commands.Param(
            name="play_now",
            description="Play the song immediately (instead of adding in line).",
            default="no",
            choices=[
                disnake.OptionChoice(
                    disnake.Localized("Yes", data={disnake.Locale.pt_BR: "Sim"}), "yes"
        ...
        repeat_amount: int = commands.Param(
            name="repetition",
            description="define the amount of repetitions.",
            default=0,
        ),
        server: str = commands.Param(
            name="server",
            desc="Use a specific music server in the search.",
            default=None,
        ),
        manual_bot_choice: str = commands.Param(
            name="selecionar_bot",
    ...
    async def check_player_queue(
        self,
        user: disnake.User,
        bot: BotCore,
        guild_id: int,
        tracks: Union[list, LavalinkPlaylist] = None,
    ):
        ...
        try:
            ...
            if len(player.queue) >= count and not (await bot.is_owner(user)):
                raise GenericError(
                    f"**A fila está cheia ({self.bot.config['QUEUE_MAX_ENTRIES']} músicas).**"
                )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Play music on a voice channel.",
        dm_permission=False,
        extras={"check_player": False},
        cooldown=play_cd,
        max_concurrency=play_mc,
    ...
    async def play(
        ...
        *,
        ...
        manual_selection: bool = commands.Param(
            name="select_manually",
            description="Choose a song manually between the results found",
            default=False,
        ),
        options: str = commands.Param(
            name="options",
            description="Playlist processing options",
            choices=playlist_opts,
            default=False,
        ),
        repeat_amount: int = commands.Param(
        ...
        if not inter.response.is_done():
            try:
                async with timeout(2.4):
                    ephemeral = await self.is_request_channel(
                        inter, data=guild_data, ignore_thread=True
                    )
        ...
        """if not inter.author.voice:
            ...
            if not (c for c in guild.channels if c.permissions_for(inter.author).connect):
                raise GenericError(f"**Você não está conectado a um voice channel, e não há canais de voz/palcos "
            ...
            msg = await func(
                embed=disnake.Embed(
                    description=f"**{inter.author.mention} entre em um voice channel para tocar sua música.**\n"
                ...
                await func(
                    embed=disnake.Embed(
                        description=f"**{inter.author.mention} operação cancelada.**\n"
                                    f"**Você demorou para conectar em um voice channel/palco.**", color=color
                    )
                )
                return
        ...
        if bot.user.id not in inter.author.voice.channel.voice_states:
            ...
            if str(inter.channel.id) == guild_data["player_controller"]["channel"]:
                ...
                try:
                    if (
                        inter.author.id
                        not in bot.music.players[guild.id].last_channel.voice_states
                    ):
                        raise DiffVoiceChannel()
                ...
                if not free_bots:
                    ...
                    if bot_count:
                        ...
                        txt = (
                            "**There are no compatible music bots on the server...**"
                    ...
                    await inter.send(
                        ...
                        components=[
                            disnake.ui.Button(
                                custom_id="bot_invite", label="Adicionar bots"
                            )
                        ],
                    )
                    return
                ...
                if bot != current_bot:
                    guild_data = await current_bot.get_data(
        ...
        await check_player_perm(
            inter=inter, bot=bot, channel=channel, guild_data=guild_data
        )

        if not guild.voice_client and not check_channel_limit(
            guild.me, inter.author.voice.channel
        ):
            raise GenericError(
                f"**The channel {inter.author.voice.channel.mention} is crowded!**"
            )
        ...
        try:
            ...
            if not guild.me.voice:
                can_connect(
                    voice_channel, guild, guild_data["check_other_bots_in_vc"], bot=bot
                )
            ...
            if static_player["channel"]:
                channel, warn_message, message = await self.check_channel(
                    guild_data, inter, channel, guild, bot
                )
            ...
            if self.bot.config["ENABLE_DISCORD_URLS_PLAYBACK"]:
                ...
                try:
                    ...
                    if attachment.size > 18000000:
                        raise GenericError(
                            "**The file you sent must have the size of 18MB or less.**"
                        )

                    if attachment.content_type not in self.audio_formats:
                        raise GenericError(
                            "**The file you sent is not a valid music file...**"
                        )
        ...
        user_data = await self.bot.get_global_data(
            ...
            fav_slashcmd = (
                ...
                + str(
                    self.bot.get_global_command_named(
                        "fav_manager",
                        cmd_type=disnake.ApplicationCommandType.chat_input,
                    ).id
                )
                + ">"
            )
        ...
        try:
            savequeue_slashcmd = (
                ...
                + str(
                    self.bot.get_global_command_named(
                        "save_queue", cmd_type=disnake.ApplicationCommandType.chat_input
                    ).id
                )
                + ">"
            )
        ...
        if not query:
            ...
            if user_data["fav_links"]:
                ...
                txt += f"`Play music or playlist you enjoyed or that you have added to your favorites.`\n"
            ...
            if user_data["integration_links"]:
                ...
                txt += (
                    f"`You have no added integration... "
                    f"Use the integrations to add YouTube channel links (or a user profile link from some music platform) to get access to all the public playlists it has.`\n"
                )

            txt += (
                f'-# To manage your integrations use the command {fav_slashcmd} and then select the "Integrations".\n'
                f"### `[💾] Save row [💾]`\n"
            )

            if os.path.isfile(
                f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl"
            ):
                txt += (
                    f"`Use music queue you saved via command` {savequeue_slashcmd}.\n"
                )
                opts.append(
                    disnake.SelectOption(
                        label="Usar fila salva",
                        value=">> [💾 Fila Salva 💾] <<",
                        emoji="💾",
                    )
                )
                ...
                txt += (
                    "`You do not have a saved music queue`\n"
            ...
            if (
                isinstance(inter, disnake.MessageInteraction)
                and not inter.response.is_done()
            ):
                await inter.response.defer(ephemeral=ephemeral)
            ...
            if guild_data["player_controller"]["fav_links"]:
                ...
                opts.append(
                    disnake.SelectOption(
                        label="Use server favorite",
                        value=">> [📌 Server Favorites 📌] <<",
                        emoji="📌",
                    )
                )
            ...
            if not select_interaction or view.selected is False:
                ...
                text = (
                    "### SEASED SELECTION TIME!"
                    if view.selected is not False
                    else "### Canceled by the user."
                )

                try:
                    await func(
                        embed=disnake.Embed(
                            description=text, color=self.bot.get_color(guild.me)
                        ),
        ...
        if query.startswith(">> [💠 Integrações 💠] <<"):
            ...
            for k, v in user_data["integration_links"].items():
                ...
                if update:
                    await self.bot.update_global_data(
                        inter.author.id, user_data, db_name=DBModel.users
                    )
                ...
                fav_opts.append(
                    {
                        ...
                        "option": disnake.SelectOption(
                            label=fix_characters(k[6:], 45),
                            value=f"> itg: {k}",
                            description=f"[💠 Integração 💠] -> {platform}",
                            emoji=emoji,
                        ),
                    }
                )
                ...
                fav_opts.append(
                    {
                        "url": v,
                        "option": disnake.SelectOption(
                            label=fix_characters(k, 45),
                            value=f"> fav: {k}",
                            description=f"[⭐ Favorite ⭐] -> {platform}",
                            emoji=emoji,
                        ),
                    }
                )
            ...
            if not user_data["last_tracks"]:
                raise GenericError(
                    "**You don't have music recorded in your history...**\n"
                    "They will appear as it is adding music via search or link."
                )
                ...
                fav_opts.append(
                    {
                        "url": v["url"],
                        "option": disnake.SelectOption(
                            label=fix_characters(name, 45),
                            value=f"> pin: {name}",
                            description="[📌 Server favorite📌]",
        ...
        if fav_opts:
            ...
            if len(fav_opts) == 1:
                ...
                check_url = lambda i: (
                    f"{i}/playlists" if (".spotify." in i or ".deezer." in i) else i
                )

                embed = disnake.Embed(
                    color=self.bot.get_color(guild.me),
                    description="\n".join(
                        f"{get_source_emoji_cfg(bot, i['url']) or ''} [`{fix_characters(i['option'].label, 45)}`]({check_url(i['url'])})"
                        for i in fav_opts
                    ),
                )
                ...
                if menu == "favs":
                    embed.description = (
                        "### `[⭐] ⠂favorites ⠂[⭐]`\n"
                        "`Play music or playlist you enjoyed or that you have added to your favorite list.`\n"
                        f"-# You can manage your favorites using the command {fav_slashcmd}.\n\n"
                ...
                try:
                    if bot.user.id != self.bot.user.id:
                        embed.set_footer(
                            text=f"Selected bot: {bot.user.display_name}",
                ...
                if not select_interaction or view.selected is False:
                    ...
                    embed.set_footer(
                        text=(
                            "⚠️ Exhausted weather!"
                            if not view.selected is False
                            else "⚠️ Canceled by the user."
                        )
                    )
                    ...
                    try:
                        ...
                        try:
                            await select_interaction.response.edit_message(
                                embed=embed, components=song_request_buttons
                            )
                        except AttributeError:
                            traceback.print_exc()
                    return
        ...
        if query.startswith("> pin: "):
            ...
            if query.startswith("> fav:"):
                ...
                if matches := spotify_regex_w_user.match(query):
                    ...
                    if not self.bot.spotify:
                        raise GenericError(
                            "**Spotify support is not available at the moment...**"
                        )
                    ...
                    if url_type != "user":
                        raise GenericError(
                            "**Link Not supported using this method...**"
                        )
                    ...
                    raise GenericError(
                        "**There is no support to this type of request at the moment...**"
                    )
                    ...
                    if not (info := self.bot.pool.integration_cache.get(query)):
                        info = await loop.run_in_executor(
                            None,
                            lambda: self.bot.pool.ytdl.extract_info(
                    ...
                    try:
                        profile_avatar = [
                            a["url"]
                            for a in info["thumbnails"]
                            if a["id"] == "avatar_uncropped"
                        ][0]
                    ...
                    try:
                        if not info["entries"]:
                            raise GenericError(
                                f"**Unavailable (or private) content:**\n{query}"
                            )
                    except KeyError:
                        raise GenericError(
                            "**An error occurred when trying to get results for the selected option...**"
                        )
                ...
                if len(info["entries"]) == 1:
                    ...
                    view = SelectInteraction(
                        ...
                        opts=[
                            disnake.SelectOption(
                                label=e["title"][:90],
                                value=f"entrie_select_{c}",
                                emoji=emoji,
                            )
                            for c, e in enumerate(info["entries"])
                        ],
                        timeout=120,
                    )

                    embed_title = (
                        f"do canal: {(info.get('title') or selected_title)[:-12]}"
                        if platform == "youtube"
                        else f"do usuário: {info.get('title') or selected_title}"
                    )

                    embeds = []

                    for page_index, page in enumerate(
                        disnake.utils.as_chunks(info["entries"], 15)
                    ):

                        embed = disnake.Embed(
                            description="\n".join(
                                f'-# ` {(15*page_index)+n+1}. `[`{i["title"]}`]({i["url"]})'
                            ...
                            name=f"Public Public Playlist {embed_title}",
                            icon_url=music_source_image(platform),
                            url=query,
                        )
            ...
            tracks = await self.check_player_queue(
                inter.author,
                bot,
                guild.id,
                self.bot.pool.process_track_cls(data["tracks"])[0],
            )
            node = await self.get_best_node(bot)
            queue_loaded = True
            source = False
            ...
            if urls:
                ...
                if (
                    not self.bot.config["ENABLE_DISCORD_URLS_PLAYBACK"]
                    and "cdn.discordapp.com/attachments/" in query
                ):
                    raise GenericError(
                        "**O suporte a links do discord está desativado.**"
                    )
                ...
                if query.startswith("https://www.youtube.com/results"):
                    try:
                        ...
                        raise GenericError(
                            f"**Não há suporte para o link informado:** {query}"
                        )
                    manual_selection = True
                    ...
                    view = SelectInteraction(
                        ...
                        opts=[
                            disnake.SelectOption(
                                label="Música",
                                emoji="🎵",
                                description="Carregar apenas a música do link.",
                                value="music",
                            ),
                            disnake.SelectOption(
                                label="Playlist",
                                emoji="🎶",
                                description="Carregar playlist com a música atual.",
                                value="playlist",
                            ),
                        ],
                        timeout=30,
                    ...
                    try:
                        if bot.user.id != self.bot.user.id:
                            embed.set_footer(
                                text=f"Bot selecionado: {bot.user.display_name}",
                                icon_url=bot.user.display_avatar.url,
                            )
                    except AttributeError:
                        pass
        ...
        if not queue_loaded:
            tracks, node = await self.get_tracks(
                query,
                inter.author,
                node=node,
                track_loops=repeat_amount,
                source=source,
                bot=bot,
            )
        ...
        try:
            ...
            try:
                ...
                if new_bot != bot or not guild_data:
                    guild_data = await new_bot.get_data(
                        guild.id, db_name=DBModel.guilds
                    )
                ...
                if static_player["channel"]:
                    channel, warn_message, message = await self.check_channel(
                        guild_data, inter, channel, guild, new_bot
                    )
        ...
        if not player:
            player = await self.create_player(
                ...
                channel=channel,
                message_inter=message_inter,
                node=node,
                modal_message_id=modal_message_id,
            )
        ...
        if isinstance(tracks, list):
            ...
            if (
                not queue_loaded
                and len(tracks) > 1
                and (tracks[0].info["sourceName"] == "deezer" or manual_selection)
            ):
                ...
                def check_song_selection(
                    i: Union[CustomContext, disnake.MessageInteraction]
                ):

                    try:
                        return (
                            i.data.custom_id == f"track_selection_{inter.id}"
                ...
                try:
                    select_interaction: disnake.MessageInteraction = (
                        await self.bot.wait_for(
                            "dropdown", timeout=45, check=check_song_selection
                        )
                    )
                except asyncio.TimeoutError:
                    raise GenericError("Exhausted weather!")
            ...
            if not isinstance(tracks, list):
                ...
                duration = (
                    time_format(tracks.duration)
                    if not tracks.is_stream
                    else "🔴 Livestream"
                )
                ...
                if queue_loaded:
                    log_text = f"{inter.author.mention} added `{len(tracks)} Music` via: {query[7:]}."
                    ...
                    for t in tracks:
                        if t.playlist_name:
                            try:
                                ...
                                tracks_playlists[t.playlist_url] = {
                    ...
                    if tracks_playlists:
                        embed_description += (
                            "\n### Charged playlists:\n"
                            + "\n".join(
                                f"[`{info['name']}`]({url}) `- {info['count']} música{'s'[:info['count']^1]}` "
                                for url, info in tracks_playlists.items()
                            )
                            + "\n"
                        )
            ...
            if tracks.tracks[0].info["sourceName"] == "youtube":
                try:
                    async with bot.session.get(
                        (oembed_url := f"https://www.youtube.com/oembed?url={query}")
                    ) as r:
                        try:
                            playlist_data = await r.json()
                        except:
                            raise Exception(f"{r.status} | {await r.text()}")
                    tracks.data["playlistInfo"]["thumb"] = playlist_data[
                        "thumbnail_url"
                    ]
                except Exception as e:
                    print(
                        f"Failure to get artwork from the playlist: {oembed_url} | {repr(e)}"
                    )
        ...
        if not is_pin:
            ...
            if loadtype == "track":
                components = [
                    disnake.ui.Button(
                        ...
                        custom_id=PlayerControls.embed_add_fav,
                    ),
                    disnake.ui.Button(
                        emoji="▶️",
                        label="Play"
                        + (
                            " agora"
                            if (player.current and player.current.autoplay)
                            else ""
                        ),
                        custom_id=PlayerControls.embed_forceplay,
                    ),
                    disnake.ui.Button(
                ...
                try:
                    self.bot.pool.enqueue_playlist_embed_cooldown.get_bucket(
                        inter
                    ).update_rate_limit()
                except:
                    pass
            ...
            await func(
                embed=embed,
                **{"components": components} if components else {"view": None},
            )
    ...
    async def fav_add_autocomplete(self, inter: disnake.Interaction, query: str):
        ...
        favs.extend(
            reversed(
                [
                    (
                        f"{rec['url']} || {rec['name']}"[:100]
                        if len(rec["url"]) < 101
                        else rec["name"][:100]
                    )
                    for rec in user_data["last_tracks"]
                ]
            )
        )
    ...
    skip_back_cd = commands.CooldownMapping.from_cooldown(
        4, 13, commands.BucketType.member
    )
    skip_back_mc = commands.MaxConcurrency(
        1, per=commands.BucketType.member, wait=False
    )

    case_sensitive_args = CommandArgparse()
    case_sensitive_args.add_argument(
        "-casesensitive",
        "-cs",
        action="store_true",
        help="Search for songs with the exact sentence in the name of the song instead of seeking word by word.",
    )
    ...
    @pool_command(
        name="skip",
        aliases=["next", "n", "s", "to jump", "skipto"],
        cooldown=skip_back_cd,
        max_concurrency=skip_back_mc,
        description=f"Skip the current song that is playing.",
        extras={"flags": case_sensitive_args},
        only_voiced=True,
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Jump to a specific line in line.",
        dm_permission=False,
        extras={"only_voiced": True},
        cooldown=skip_back_cd,
        max_concurrency=skip_back_mc,
    ...
    async def skipto(
        ...
        query: str = commands.Param(
            name="name", description="Music name (complete or part of it)."
        ),
        case_sensitive: bool = commands.Param(
            name="exact_name",
            default=False,
            description="Search for songs with the exact sentence in the name of the song instead of seeking word by word.",
        ),
    ):

        await self.skip.callback(
            self=self, inter=inter, query=query, case_sensitive=case_sensitive
        )
        ...
        *,
        ...
        play_only: str = commands.Param(
            name=disnake.Localized(
                "play_only", data={disnake.Locale.pt_BR: "play_only"}
            ),
            choices=[
                disnake.OptionChoice(
                    disnake.Localized("Yes", data={disnake.Locale.pt_BR: "Sim"}), "yes"
            ...
            description="Just play the music immediately (without rotating the flia)",
            default="no",
        ),
        case_sensitive: bool = commands.Param(
            name="nome_exato",
        ...
        if query:
            ...
            try:
                index = queue_track_index(
                    inter, bot, query, case_sensitive=case_sensitive
                )[0][0]
            except IndexError:
                raise GenericError(
                    f"**There are no songs in line with the name: {query}**"
                )
            ...
            player.set_command_log(
                emoji="⤵️", text=f"{inter.author.mention} jumped to the current music."
            )
            ...
            if (
                isinstance(inter, disnake.MessageInteraction)
                and inter.data.custom_id == "queue_track_selection"
            ):
            ...
            if isinstance(inter, disnake.MessageInteraction):
                player.set_command_log(
                    text=f"{inter.author.mention} skipped the song.", emoji="⏭️"
                )
                if not inter.response.is_done():
                    try:
                        await inter.response.defer()
                ...
                player.set_command_log(
                    emoji="⏭️", text=f"{inter.author.mention} skipped the song."
                )
    ...
    @commands.max_concurrency(1, commands.BucketType.member)
    @commands.slash_command(
        description=f"{desc_prefix}Back to the previous song.",
        dm_permission=False,
        extras={"only_voiced": True},
        cooldown=skip_back_cd,
        max_concurrency=skip_back_mc,
    ...
    async def back(self, inter: disnake.AppCmdInter):
        ...
        if not len(player.queue) and (player.keep_connected or not len(player.played)):
            ...
            await self.interaction_message(
                inter, "He returned to the beginning of the song.", emoji="⏪"
            )
            return
        ...
        if isinstance(inter, disnake.MessageInteraction):
            interaction = inter
            player.set_command_log(
                text=f"{inter.author.mention} He returned to the current music.",
                emoji="⏮️",
            )
            await inter.response.defer()
        else:
            ...
            txt = [
                "He returned to the current music.",
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Vote to skip current music.",
        extras={"only_voiced": True},
        dm_permission=False,
    )
    async def voteskip(self, inter: disnake.AppCmdInter):
        ...
        if inter.author.id in player.votes:
            raise GenericError("**You have already voted to skip the current music.**")

        embed.colour = self.bot.get_color(guild.me)

        txt = [
            f"voted to skip current music (votes: {len(player.votes) + 1}/{self.bot.config['VOTE_SKIP_AMOUNT']}).",
    ...
    @pool_command(
        ...
        only_voiced=True,
        cooldown=volume_cd,
        max_concurrency=volume_mc,
        usage="{prefix}{cmd} [nivel]\nEx: {prefix}{cmd} 50",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Adjust music volume.",
        extras={"only_voiced": True},
        cooldown=volume_cd,
        max_concurrency=volume_mc,
        dm_permission=False,
    ...
    async def volume(
        ...
        *,
        value: int = commands.Param(
            name="nível",
            description="level between 5 to 150",
            min_value=5.0,
            max_value=150.0,
        ),
    ):
        ...
        if value is None:
            ...
            await inter.send(
                embed=embed, ephemeral=await self.is_request_channel(inter), view=view
            )
            await view.wait()
            ...
            raise GenericError("The volume must be between **5** to **150**.")

        await player.set_volume(value)

        txt = [
            f"adjusted the volume to **{value}%**",
            f"🔊 **⠂{inter.author.mention} adjusted the volume to {value}%**",
        ]
        await self.interaction_message(inter, txt, emoji="🔊")

    pause_resume_cd = commands.CooldownMapping.from_cooldown(
        2, 7, commands.BucketType.member
    )
    pause_resume_mc = commands.MaxConcurrency(
        1, per=commands.BucketType.member, wait=False
    )
    ...
    @pool_command(
        ...
        only_voiced=True,
        cooldown=pause_resume_cd,
        max_concurrency=pause_resume_mc,
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Pause the music.",
        extras={"only_voiced": True},
        cooldown=pause_resume_cd,
        max_concurrency=pause_resume_mc,
        dm_permission=False,
    ...
    async def pause(self, inter: disnake.AppCmdInter):
        ...
        if player.paused:
            raise GenericError("**The song is already paused.**")
    ...
    @pool_command(
        name="resume",
        aliases=["unpause"],
        description="Return/Dispause the music.",
        only_voiced=True,
        cooldown=pause_resume_cd,
        max_concurrency=pause_resume_mc,
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Return/Display the Music.",
        dm_permission=False,
        extras={"only_voiced": True},
        cooldown=pause_resume_cd,
        max_concurrency=pause_resume_mc,
    ...
    async def resume(self, inter: disnake.AppCmdInter):
        ...
        txt = [
            "retomou a música.",
            f"▶️ **⠂{inter.author.mention} disseminate the music.**",
        ]
        await self.interaction_message(inter, txt, rpc_update=True, emoji="▶️")
        await player.update_stage_topic()
    ...
    @pool_command(
        name="seek",
        aliases=["sk"],
        description="Advance/resume music for a specific time.",
        only_voiced=True,
        cooldown=seek_cd,
        max_concurrency=seek_mc,
    ...
    async def seek(
        ...
        position: str = commands.Param(
            name="tempo", description="Time to advance/Back (ex: 1:45 / 40 / 0:30)"
        ),
    ):
        ...
        if milliseconds > player.position:
            ...
            txt = [
                ...
                f"{emoji} **⠂{inter.author.mention} He advanced the time of music to:** `{time_format(milliseconds)}`",
            ]
            ...
            txt = [
                f"the time of the music came back to: `{time_format(milliseconds)}`",
                f"{emoji} **⠂{inter.author.mention} the time of the music came back to:** `{time_format(milliseconds)}`",
            ]
    ...
    @check_voice()
    @pool_command(
        description=f"Select Repetition Mode Between: Current / Line / Disable / Quantity (using numbers).",
        only_voiced=True,
        cooldown=loop_cd,
        max_concurrency=loop_mc,
            ...
            msg = await ctx.send(
                ...
                components=[
                    disnake.ui.Select(
                        ...
                        options=[
                            disnake.SelectOption(
                                label="Current music", value="current"
                            ),
                            disnake.SelectOption(label="Player queue", value="queue"),
                            disnake.SelectOption(label="Repetition", value="off"),
                        ],
                    )
                ],
            )
    ...
    async def loop_mode(
        ...
        mode: str = commands.Param(
            ...
            choices=[
                disnake.OptionChoice(
                    disnake.Localized(
                        "Current", data={disnake.Locale.pt_BR: "Current music"}
                    ),
                    "current",
                ),
                disnake.OptionChoice(
                    disnake.Localized("Queue", data={disnake.Locale.pt_BR: "queue"}),
                    "queue",
                ),
                disnake.OptionChoice(
                    disnake.Localized("Off", data={disnake.Locale.pt_BR: "Disable"}),
                    "off",
                ),
            ],
        ),
        ...
        if mode == player.loop:
            raise GenericError("**The selected repeat mode is already active ...**")
        ...
        if mode == "off":
            ...
            emoji = "⭕"
            txt = [
                "disabled the repetition.",
                f"{emoji} **⠂{inter.author.mention}disabled the repetition.**",
            ]
            ...
            emoji = "🔂"
            txt = [
                "activated the repetition of current music.",
                f"{emoji} **⠂{inter.author.mention} activated the repetition of current music.**",
            ]

        else:  # queue
            emoji = "🔁"
            txt = [
                "activated the repetition of the line.",
                f"{emoji} **⠂{inter.author.mention} activated the repetition of the line.**",
            ]
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Define the amount of repetitions of current music.",
        extras={"only_voiced": True},
        cooldown=loop_cd,
        max_concurrency=loop_mc,
        dm_permission=False,
    ...
    @pool_command(
        name="remove",
        aliases=["r", "del"],
        description="Remove a specific line from the line.",
        only_voiced=True,
        max_concurrency=remove_mc,
        extras={"flags": case_sensitive_args},
        usage="{prefix}{cmd} [nome]\nEx: {prefix}{cmd} sekai",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Remove a specific line from the line.",
        extras={"only_voiced": True},
        max_concurrency=remove_mc,
        dm_permission=False,
    )
    async def remove(
        ...
        txt = [
            f"removed the music [`{(fix_characters(track.title, 25))}`](<{track.uri or track.search_uri}>) da fila.",
    ...
    queue_manipulation_cd = commands.CooldownMapping.from_cooldown(
        2, 15, commands.BucketType.guild
    )
    ...
    @pool_command(
        ...
        aliases=["re-add", "rdd"],
        only_voiced=True,
        cooldown=queue_manipulation_cd,
        max_concurrency=remove_mc,
        description="Reading the songs played in queue.",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Reading the songs played in queue.",
        dm_permission=False,
        extras={"only_voiced": True},
        cooldown=queue_manipulation_cd,
        max_concurrency=remove_mc,
    ...
    async def readd_songs(self, inter: disnake.AppCmdInter):
        ...
        txt = [
            f"re-added [{qsize}] music{(s:='s'[:qsize^1])} played in the queue.",
            f"🎶 **⠂{inter.author.mention} re-added {qsize} music in queue.**",
        ]
    ...
    async def rotate(
        ...
        if index <= 0:
            raise GenericError(
                f"**A música **[`{track.title}`](<{track.uri or track.search_uri}>) já é a próxima da fila."
            )
    ...
    song_request_thread_cd = commands.CooldownMapping.from_cooldown(
        1, 120, commands.BucketType.guild
    )
    ...
    @pool_command(
        name="songrequestthread",
        aliases=["songrequest", "srt"],
        only_voiced=True,
    ...
    @commands.slash_command(
        extras={"only_voiced": True},
        cooldown=song_request_thread_cd,
        dm_permission=False,
    ...
    async def song_request_thread(self, inter: disnake.AppCmdInter):
        ...
        if not self.bot.intents.message_content:
            raise GenericError(
                "**Atualmente não tenho a intent de message-content para conferir "
        ...
        if player.static:
            raise GenericError(
                "**Você não pode usar esse comando com um canal de song-request configurado.**"
            )
        ...
        if not isinstance(player.text_channel, disnake.TextChannel):
            raise GenericError(
                "**O player-controller está ativo em um canal incompatível com "
                "criação de thread/conversa.**"
            )

        if not player.controller_mode:
            raise GenericError(
                "**A skin/aparência atual não é compatível com o sistem de song-request "
                "via thread/conversa\n\n"
        ...
        if not player.text_channel.permissions_for(guild.me).send_messages:
            raise GenericError(
                f"**{bot.user.mention} não possui permissão enviar mensagens no canal {player.text_channel.mention}.**"
            )

        if not player.text_channel.permissions_for(guild.me).create_public_threads:
            raise GenericError(
                f"**{bot.user.mention} não possui permissão de criar tópicos públicos.**"
            )

        if not [
            m
            for m in player.guild.me.voice.channel.members
            if not m.bot
            and player.text_channel.permissions_for(m).send_messages_in_threads
        ]:
            raise GenericError(
                f"**Não há membros no canal <#{player.channel_id}> com permissão de enviar mensagens "
                f"em tópicos no canal {player.text_channel.mention}"
            )

        await inter.response.defer(ephemeral=True)

        thread = await player.message.create_thread(
            name=f"{bot.user.name} temp. song-request", auto_archive_duration=10080
        )
    ...
    nightcore_cd = commands.CooldownMapping.from_cooldown(
        1, 7, commands.BucketType.guild
    )
    ...
    @pool_command(
        ...
        aliases=["nc"],
        only_voiced=True,
        cooldown=nightcore_cd,
        max_concurrency=nightcore_mc,
        description="Activate/Disable the nightcore effect (Música acelerada com tom mais agudo).",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Activate/Disable the nightcore effect (Música acelerada com tom mais agudo).",
        extras={"only_voiced": True},
        cooldown=nightcore_cd,
        max_concurrency=nightcore_mc,
        dm_permission=False,
    ...
    async def nightcore(self, inter: disnake.AppCmdInter):
        ...
        txt = [
            f"{txt} the nightcore effect.",
            f"🇳 **⠂{inter.author.mention} {txt} the nightcore effect.**",
        ]
    ...
    @commands.command(
        name="nowplaying",
        aliases=["np", "npl", "current", "tocando", "playing"],
        description="Exibir informações da música que você está ouvindo no momento.",
        cooldown=np_cd,
    )
    ...
    @commands.slash_command(
        description=f"{desc_prefix}Exibir info da música que que você está ouvindo (em qualquer servidor).",
        dm_permission=False,
        cooldown=np_cd,
    ...
    async def now_playing(self, inter: disnake.AppCmdInter):
        ...
        if not player:
            ...
            if isinstance(inter, CustomContext) and not (
                ...
                raise GenericError(
                    "**Você deve estar conectado em um voice channel do servidor atual onde há player ativo...**\n"
            ...
            for bot in self.bot.pool.get_guild_bots(inter.guild_id):
                ...
                for player_id in bot.music.players:
                    ...
                    if (
                        inter.author.id
                        in (p := bot.music.players[player_id]).last_channel.voice_states
                    ):
                        player = p
                        break
        ...
        if not player:
            raise GenericError(
                "**Você deve estar conectado em um voice channel com player ativo...**"
            )

        if not player.current:
            raise GenericError(
                f"**No momento não estou tocando algo no canal {player.last_channel.mention}**"
            )

        guild_data = await player.bot.get_data(inter.guild_id, db_name=DBModel.guilds)

        ephemeral = (
            player.guild.id != inter.guild_id
            and not await player.bot.is_owner(inter.author)
        ) or await self.is_request_channel(inter, data=guild_data)
        ...
        components = [
            disnake.ui.Button(
                custom_id=f"np_{inter.author.id}", label="Atualizar", emoji="🔄"
            )
        ]
        ...
        if player.guild_id != inter.guild_id:
            ...
            if (
                ...
                and (
                    listeners := len(
                        [
                            m
                            for m in player.last_channel.members
                            if not m.bot and (not m.voice.self_deaf or not m.voice.deaf)
                        ]
                    )
                )
                > 1
            ...
            try:
                footer_kw = {
                    "icon_url": player.guild.icon.with_static_format("png").url
                }
            except AttributeError:
                pass

            footer_kw["text"] = (
                f"No servidor: {player.guild.name} [ ID: {player.guild.id} ]"
            )
            ...
            try:
                if player.bot.user.id != self.bot.user.id:
                    footer_kw["text"] = (
                        f"Bot selecionado: {player.bot.user.display_name}"
                    )
            ...
            if player.guild_id == inter.guild_id:
                ...
                txt += f"### 🎶 ⠂Próximas músicas ({(qsize := len(player.queue + player.queue_autoplay))}):\n" + (
                    ...
                    for n, t in enumerate(
                        itertools.islice(player.queue + player.queue_autoplay, 3)
                    )
                )
        ...
        if player.static and player.guild_id == inter.guild_id:
            if player.message:
                components.append(
                    disnake.ui.Button(
                        url=player.message.jump_url,
                        label="Ir p/ player-controller",
                        emoji="🔳",
                    )
                )
        ...
        embed = disnake.Embed(
            description=txt, color=self.bot.get_color(player.guild.me)
        )
        ...
        embed.set_author(
            name=(
                (
                    "⠂Now Playing:"
                    if inter.guild_id == player.guild_id
                    else "Você está ouvindo agora:"
                )
                if not player.paused
                else "⠂Música atual:"
            ),
        ...
        if isinstance(inter, disnake.MessageInteraction):
            await inter.response.edit_message(
                inter.author.mention, embed=embed, components=components
            )
        else:
            await inter.send(
    ...
    controller_cd = commands.CooldownMapping.from_cooldown(
        1, 10, commands.BucketType.member
    )
    controller_mc = commands.MaxConcurrency(
        1, per=commands.BucketType.member, wait=False
    )
    ...
    @pool_command(
        ...
        aliases=["ctl"],
        only_voiced=True,
        cooldown=controller_cd,
        max_concurrency=controller_mc,
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Enviar player controller para um canal específico/atual.",
        extras={"only_voiced": True},
        cooldown=controller_cd,
        max_concurrency=controller_mc,
        dm_permission=False,
    ...
    async def controller(self, inter: disnake.AppCmdInter):
        ...
        if player.static:
            raise GenericError(
                "Esse comando não pode ser usado no modo fixo do player."
            )

        if player.has_thread:
            raise GenericError(
                "**Esse comando não pode ser usado com uma conversa ativa na "
    ...
    @commands.user_command(
        name=disnake.Localized("Add DJ", data={disnake.Locale.pt_BR: "Adicionar DJ"}),
        extras={"only_voiced": True},
        dm_permission=False,
    )
    ...
    async def add_dj(
        ...
        *,
        user: disnake.User = commands.Param(
            name="membro", description="Membro a ser adicionado."
        ),
    ):
        ...
        text = [
            f"adicionou {user.mention} à lista de DJ's.",
            f"🎧 **⠂{inter.author.mention} adicionou {user.mention} na lista de DJ's.**",
        ]

        if (player.static and channel == player.text_channel) or isinstance(
            inter.application_command, commands.InvokableApplicationCommand
        ):
            await inter.send(
                f"{user.mention} adicionado à lista de DJ's!{player.controller_link}"
            )
        ...
        *,
        ...
        if user.id == player.player_creator:
            if inter.author.guild_permissions.manage_guild:
                ...
                raise GenericError(
                    f"**O membro {user.mention} é o criador do player.**"
                )
        ...
        text = [
            f"removeu {user.mention} da lista de DJ's.",
    ...
    @pool_command(
        ...
        aliases=["leave", "parar"],
        only_voiced=True,
        description="Parar o player e me desconectar do voice channel.",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Parar o player e me desconectar do voice channel.",
        extras={"only_voiced": True},
        dm_permission=False,
    )
    ...
    async def save_queue(self, inter: disnake.AppCmdInter):
        ...
        if len(tracks) < 3:
            raise GenericError(
                f"**É necessário ter no mínimo 3 músicas pra salvar (atual e/ou na fila)**"
            )
        ...
        async with aiofiles.open(
            f"./local_database/saved_queues_v1/users/{inter.author.id}.pkl", "wb"
        ) as f:
            await f.write(
                zlib.compress(
                    pickle.dumps(
        ...
        embed.set_footer(
            text="Nota: Esse é um recurso muito experimental, a fila salva pode sofrer alterações ou ser "
    ...
    @check_voice()
    @commands.slash_command(
        name="queue", extras={"only_voiced": True}, dm_permission=False
    )
    ...
    @q.sub_command(
        name="shuffle",
        description=f"{desc_prefix}Mix the songs in the queue",
        extras={"only_voiced": True},
        cooldown=queue_manipulation_cd,
        max_concurrency=remove_mc,
    )
    async def shuffle_(self, inter: disnake.AppCmdInter):
        ...
        if len(player.queue) < 3:
            raise GenericError(
                "**A fila tem que ter no mínimo 3 músicas para ser misturada.**"
            )
    ...
    async def reverse(self, inter: disnake.AppCmdInter):
        ...
        if len(player.queue) < 2:
            raise GenericError(
                "**A fila tem que ter no mínimo 2 músicas para inverter a ordem.**"
            )
        ...
        await self.interaction_message(
            inter,
            txt=[
                "inverteu a ordem das músicas na fila.",
                f"🔄 **⠂{inter.author.mention} inverteu a ordem das músicas na fila.**",
            ],
            emoji="🔄",
        )

    queue_show_mc = commands.MaxConcurrency(
        1, per=commands.BucketType.member, wait=False
    )
    ...
    @pool_command(
        name="queue",
        aliases=["q", "fila"],
        description="Exibir as músicas que estão na fila.",
        only_voiced=True,
        max_concurrency=queue_show_mc,
    )
    ...
    @pool_command(
        name="clear",
        aliases=["limpar", "clearqueue"],
        description="Limpar a fila de música.",
        only_voiced=True,
        extras={"flags": clear_flags},
        cooldown=queue_manipulation_cd,
        max_concurrency=remove_mc,
    )
    async def clear_legacy(self, ctx: CustomContext, *, flags: str = ""):
        ...
        await self.clear.callback(
            ...
            user=(
                await commands.MemberConverter().convert(ctx, " ".join(args.member))
                if args.member
                else None
            ),
    ...
    async def clear(
        self,
        inter: disnake.AppCmdInter,
        song_name: str = commands.Param(
            name="nome",
            description="Incluir um nome que tiver na música.",
            default=None,
        ),
        song_author: str = commands.Param(
            name="uploader",
        ...
        duplicates: bool = commands.Param(
            name="duplicados", description="Incluir músicas duplicadas", default=False
        ),
        playlist: str = commands.Param(
            description="Incluir nome que tiver na playlist.", default=None
        ),
        min_duration: str = commands.Param(
            name="duração_inicial",
            description="incluir músicas com duração acima/igual (ex. 1:23).",
            default=None,
        ),
        max_duration: str = commands.Param(
            name="duração_máxima",
        ...
        amount: int = commands.Param(
            name="quantidade",
            description="Quantidade de músicas para mover.",
            min_value=0,
            max_value=99,
            default=None,
        ),
        range_start: int = commands.Param(
            name="posição_inicial",
            description="incluir músicas da fila a partir da posição específicada.",
            min_value=1.0,
            max_value=500.0,
            default=0,
        ),
        range_end: int = commands.Param(
            name="posição_final",
        ...
        absent_members: bool = commands.Param(
            name="membros_ausentes",
        ...
        if not filters and not range_start and not range_end:
            player.queue.clear()
            txt = [
                "limpou a fila de música.",
                f"♻️ **⠂{inter.author.mention} limpou a fila de música.**",
            ]
            ...
            if range_start > 0 and range_end > 0:
                ...
                if range_start >= range_end:
                    raise GenericError(
                        "**A posição final deve ser maior que a posição inicial!**"
                    )

                song_list = list(player.queue)[range_start - 1 : -(range_end - 1)]
                txt.append(
                    f"**Posição inicial da fila:** `{range_start}`\n"
            ...
            for t in song_list:
                ...
                if "duplicates" in temp_filter:
                    if (
                        title := f"{t.author} - {t.title}".lower()
                    ) in duplicated_titles:
                ...
                if (
                    "song_author" in temp_filter
                    and song_author.lower() in t.author.replace("️", "").lower()
                ):
                ...
                if "user" in temp_filter and user.id == t.requester:
                    ...
                    "absent_members" in temp_filter
                    and t.requester not in player.guild.me.voice.channel.voice_states
                ):
                ...
                if "playlist" in temp_filter:
                    if playlist == t.playlist_name.replace("️", "") or (
                        isinstance(inter, CustomContext)
                        and playlist.lower() in t.playlist_name.replace("️", "").lower()
                    ):
                        playlist_link = (
                            f"[`{fix_characters(t.playlist_name)}`](<{t.playlist_url}>)"
                        )
            ...
            try:
                ...
                txt.append(
                    f"**Inclui nome no uploader/artista:** `{fix_characters(song_author)}`"
                )
            except:
                pass
            ...
            try:
                ...
                txt.append(
                    f"**Com duração inicial/igual:** `{time_format(min_duration)}`"
                )
            except:
                pass
            ...
            msg_txt = (
                f"### ♻️ ⠂{inter.author.mention} removeu {deleted_tracks} música{'s'[:deleted_tracks^1]} da fila:\n"
                + "\n".join(
                    f"[`{fix_characters(t.title, 45)}`](<{t.uri}>)"
                    for t in tracklist[:7]
                )
            )
            ...
            txt = [
                f"removeu {deleted_tracks} música{'s'[:deleted_tracks^1]} da fila via clear.",
                msg_txt,
            ]
    ...
    @pool_command(
        name="move",
        aliases=["movequeue", "moveadv", "moveadvanced", "moveq", "mq", "mv", "mover"],
        description="Mover músicas da fila.",
        only_voiced=True,
        extras={"flags": move_queue_flags},
        cooldown=queue_manipulation_cd,
        max_concurrency=remove_mc,
    )
    async def move_legacy(
        self, ctx: CustomContext, position: Optional[int] = None, *, flags: str = ""
    ):
    ...
    async def move(
        ...
        position: int = commands.Param(
            name="posição",
            description="Posição de destino na fila (Opcional).",
            min_value=1,
            max_value=900,
            default=1,
        ),
    ...
    async def do_move(
        ...
        if range_start > 0 and range_end > 0:
            ...
            indexes = queue_track_index(
                inter, bot, song_name, match_count=1, case_sensitive=case_sensitive
            )
        ...
        components = [
            disnake.ui.Button(
                emoji="▶️", label="Tocar agora", custom_id=PlayerControls.embed_forceplay
            ),
        ]
        ...
        if indexes:
            ...
            moved_tracks_txt = (
                moved_tracks
                if moved_tracks == 1
                else f"[{position}-{position+moved_tracks-1}]"
            )

            msg_txt = (
                f"### ↪️ ⠂{inter.author.mention} moveu {moved_tracks} música{'s'[:moved_tracks^1]} para a posição {moved_tracks_txt} da fila:\n"
                + "\n".join(
                    f"`{position+n}.` [`{fix_characters(t.title, 45)}`](<{t.uri}>)"
                    for n, t in enumerate(tracklist[:7])
                )
            )
            ...
            txt = [
                f"moveu {moved_tracks} música{'s'[:moved_tracks^1]} para a posição **[{position}]** da fila.",
                msg_txt,
            ]
        ...
        return list(
            set(
                [
                    ...
                    and query.lower() in track.playlist_name.lower()
                ]
            )
        )[:20]
    ...
    async def queue_tracks(self, inter: disnake.AppCmdInter, query: str):
        ...
        return (
            results
            or [
                f"{track.title[:81]} || ID > {track.unique_id}"
                for n, track in enumerate(player.queue + player.queue_autoplay)
    ...
    async def queue_author(self, inter: disnake.Interaction, query: str):
        ...
        if not query:
            ...
            return list(
                set(
                    [
                        track.authors_string
                        for track in player.queue
                        if query.lower() in track.authors_string.lower()
                    ]
                )
            )[:20]

    restrict_cd = commands.CooldownMapping.from_cooldown(
        2, 7, commands.BucketType.member
    )
    ...
    @pool_command(
        name="restrictmode",
        aliases=["rstc", "restrict", "restrito", "modorestrito"],
        only_voiced=True,
        cooldown=restrict_cd,
        max_concurrency=restrict_mc,
        description="Activate/Disable restricted mode de comandos que requer DJ/Staff.",
    )
    ...
    @check_voice()
    @commands.slash_command(
        description=f"{desc_prefix}Activate/Disable restricted mode de comandos que requer DJ/Staff.",
        extras={"only_voiced": True},
        cooldown=restrict_cd,
        max_concurrency=restrict_mc,
        dm_permission=False,
    ...
    async def restrict_mode(self, inter: disnake.AppCmdInter):
        ...
        text = [
            f"{msg[0]} restricted mode de comandos do player (que requer DJ/Staff).",
            f"{msg[1]} **⠂{inter.author.mention} {msg[0]} restricted mode de comandos do player (que requer DJ/Staff).**",
        ]

        await self.interaction_message(inter, text, emoji=msg[1])

    nonstop_cd = commands.CooldownMapping.from_cooldown(
        2, 15, commands.BucketType.member
    )
    ...
    @pool_command(
        ...
        only_voiced=True,
        cooldown=nonstop_cd,
        max_concurrency=nonstop_mc,
        description="Activate/Disable o modo 24/7 do player (Em testes).",
    )
    ...
    @commands.slash_command(
        name="247",
        description=f"{desc_prefix}Activate/Disable o modo 24/7 do player (Em testes).",
        default_member_permissions=disnake.Permissions(manage_guild=True),
        dm_permission=False,
        extras={"only_voiced": True},
        cooldown=nonstop_cd,
    ...
    autoplay_cd = commands.CooldownMapping.from_cooldown(
        2, 15, commands.BucketType.member
    )
    ...
    @pool_command(
        ...
        aliases=["ap", "aplay"],
        only_voiced=True,
        cooldown=autoplay_cd,
        max_concurrency=autoplay_mc,
        description="Activate/Disable autoplay ao acabar as músicas da fila.",
    )
    ...
    @commands.slash_command(
        name="autoplay",
        description=f"{desc_prefix}Activate/Disable autoplay ao acabar as músicas da fila.",
        extras={"only_voiced": True},
        cooldown=autoplay_cd,
        max_concurrency=autoplay_mc,
        dm_permission=False,
    ...
    async def change_node(
        ...
        embed = disnake.Embed(
            description=f"**O player foi migrado para o servidor de música:** `{node}`",
    ...
    async def node_suggestions(self, inter: disnake.Interaction, query: str):
        ...
        if not query:
            return [
                ...
                if n != node and n.available and n.is_available
            ]
        ...
        return [
            ...
            for n in bot.music.nodes.values()
            if n != node
            and query.lower() in n.identifier.lower()
            and n.available
            and n.is_available
        ]
    ...
    @commands.command(
        aliases=["puptime"],
        description="Ver informações de tempo que o player está ativo no servidor.",
    )
    ...
    fav_import_export_cd = commands.CooldownMapping.from_cooldown(
    ...
    @commands.command(
        ...
        description="Gerenciar seus favoritos/integrações e links do server.",
        cooldown=fav_cd,
    )
    ...
    async def fav_manager(self, inter: disnake.AppCmdInter):
        ...
        if isinstance(inter, CustomContext):
            ...
            if inter.invoked_with in ("serverplaylist", "spl", "svp", "svpl") and (
                inter.author.guild_permissions.manage_guild
                or await bot.is_owner(inter.author)
            ):
            ...
            global_data = await bot.get_global_data(
                inter.guild_id, db_name=DBModel.guilds
            )
        ...
        view = FavMenuView(
            ...
            prefix=prefix,
            mode=mode,
            is_owner=await bot.is_owner(inter.author),
        )
        ...
        if not txt:
            raise GenericError(
                "**Não há suporte a esse recurso no momento...**\n\n"
    ...
    async def resume_players_ready(self):
        ...
        for guild_id in list(self.bot.music.players):
            ...
            try:
                ...
                if not vc:
                    print(
                        f"{self.bot.user} - {player.guild.name} [{guild_id}] - Player finalizado por falta de voice channel"
                    )
                    try:
                        await player.destroy()
                    except:
                ...
                if not player.is_paused and not player.is_playing:
                    await player.process_next()
                print(
                    f"{self.bot.user} - {player.guild.name} [{guild_id}] - Player Reconectado no voice channel"
                )
            except:
                traceback.print_exc()

    async def is_request_channel(
        self,
        ctx: Union[disnake.AppCmdInter, disnake.MessageInteraction, CustomContext],
        *,
        data: dict = None,
        ignore_thread=False,
    ) -> bool:
        ...
        try:
            ...
            if (
                isinstance(channel_ctx, disnake.Thread)
                and player.text_channel == channel_ctx.parent
            ):
            ...
            if (
                isinstance(channel_ctx, disnake.Thread)
                and channel == channel_ctx.parent
            ):
                return not ignore_thread
    ...
    async def check_channel(
        ...
        try:
            channel_db = bot.get_channel(
                int(static_player["channel"])
            ) or await bot.fetch_channel(int(static_player["channel"]))
            ...
            if channel_db.id != channel.id:
                ...
                try:
                    if isinstance(channel_db, disnake.Thread):
                        ...
                        if not channel_db.parent:
                            await self.reset_controller_db(
                                inter.guild_id, guild_data, inter
                            )
                            channel_db = None
                            ...
                            if channel_db.owner != bot.user.id:
                                ...
                                if (
                                    not isinstance(
                                        channel_db.parent, disnake.ForumChannel
                                    )
                                    or not channel_db.parent.permissions_for(
                                        channel_db.guild.me
                                    ...
                                    if (
                                        not thread
                                        and guild.me.guild_permissions.read_message_history
                                    ):
                                        async for (
                                            t
                                        ) in channel_db.parent.archived_threads(
                                            limit=100
                                        ):
                                            if t.owner_id == bot.user.id:
                                                try:
                                                    message = await t.fetch_message(
                                                        t.id
                                                    )
                                                except disnake.NotFound:
                                                    continue
                                    ...
                                    if not thread:
                                        thread_wmessage = (
                                            await channel_db.parent.create_thread(
                            ...
                            if (
                                channel_db.locked
                                and channel_db.permissions_for(guild.me).manage_threads
                            ):
                                thread_kw.update({"locked": False, "archived": False})

                            elif (
                                channel_db.archived
                                and channel_db.owner_id == bot.user.id
                            ):
                            ...
                            if thread_kw:
                                ...
                                warn_message = (
                                    f"**{bot.user.mention} não possui permissão de gerenciar tópicos "
                ...
                if channel_db:
                    ...
                    if isinstance(channel, disnake.Thread):
                        send_message_perm = (
                            getattr(channel_db, "parent", channel_db)
                            .permissions_for(channel.guild.me)
                            .send_messages_in_threads
                        )
                    else:
                        send_message_perm = channel_db.permissions_for(
                            channel.guild.me
                        ).send_messages
    ...
    async def guild_pin(self, interaction: disnake.MessageInteraction):
        ...
        if not interaction.user.voice:
            await interaction.send(
                "Você deve entrar em um voice channel para usar isto.", ephemeral=True
            )
            return

        await interaction.response.defer(ephemeral=True)

        guild_data = await self.bot.get_data(
            interaction.guild_id, db_name=DBModel.guilds
        )
        ...
        try:
            ...
            await interaction.send(
                "**O item selecionado não foi encontrado na base de dados...**",
                ephemeral=True,
            )
            await send_idle_embed(
                interaction.message, bot=self.bot, guild_data=guild_data, force=True
            )
            return
    ...
    async def check_stage_title(self, inter, bot: BotCore, player: LavalinkPlayer):
        ...
        if (
            player.stage_title_event
            and (
                time_ := int(
                    (disnake.utils.utcnow() - player.start_time).total_seconds()
                )
            )
            < time_limit
            and not (await bot.is_owner(inter.author))
        ):
            raise GenericError(
    ...
    async def player_controller(
        self, interaction: disnake.MessageInteraction, control: str, **kwargs
    ):
        ...
        if control in (
            ...
            try:
                ...
                for b in sorted(
                    self.bot.pool.get_guild_bots(interaction.guild_id),
                    key=lambda b: b.identifier,
                    reverse=True,
                ):
                    ...
                    if (
                        ...
                        in p.guild.me.voice.channel.voice_states
                    ):
                ...
                if not author.voice:
                    raise GenericError(
                        "Você deve entrar em um voice channel pra usar esse botão...."
                    )
                ...
                await check_player_perm(
                    inter=interaction, bot=bot, channel=interaction.channel
                )
                ...
                if control == PlayerControls.embed_enqueue_playlist:
                    ...
                    if retry_after := self.bot.pool.enqueue_playlist_embed_cooldown.get_bucket(
                        interaction
                    ).update_rate_limit():
                        raise GenericError(
                    ...
                    await self.check_player_queue(
                        interaction.author, bot, interaction.guild_id
                    )
                    result, node = await self.get_tracks(
                        url, author, source=False, node=player.node, bot=bot
                    )
                    result = await self.check_player_queue(
                        interaction.author, bot, interaction.guild_id, tracks=result
                    )
                    player.queue.extend(result.tracks)
                    await interaction.send(
                        f"{interaction.author.mention}, a playlist [`{result.name}`](<{url}>) foi adicionada com sucesso!{player.controller_link}",
                        ephemeral=True,
                    )
                    if not player.is_connected:
                    ...
                    if player:
                        ...
                        if (
                            control == PlayerControls.embed_forceplay
                            and player.current
                            and (
                                player.current.uri.startswith(url)
                                or url.startswith(player.current.uri)
                            )
                        ):
                            await self.check_stage_title(
                                inter=interaction, bot=bot, player=player
                            )
                            await player.seek(0)
                            player.set_command_log(
                                "voltou para o início da música.", emoji="⏪"
                            )
                            ...
                            if not track:
                                ...
                                if not track:
                                    ...
                                    for t in list(player.failed_tracks):
                                        if t.uri.startswith(url) or url.startswith(
                                            t.uri
                                        ):
                                            track = t
                    ...
                    if not seek_status:
                        ...
                        if not track:
                            ...
                            if retry_after := self.bot.pool.enqueue_track_embed_cooldown.get_bucket(
                                interaction
                            ).update_rate_limit():
                                raise GenericError(
                            ...
                            result, node = await self.get_tracks(
                                url, author, source=False, node=node, bot=bot
                            )
                        ...
                        if control == PlayerControls.embed_enqueue_track:
                            ...
                            player.update = True
                            await interaction.send(
                                f"{author.mention}, a música [`{track.title}`](<{track.uri}>) foi adicionada na fila.{player.controller_link}",
                                ephemeral=True,
                            )
                            if not player.is_connected:
                                await player.connect(vc_id)
                            ...
                            if not player.is_connected:
                                await player.connect(vc_id)
                            await self.process_music(
                                inter=interaction, player=player, force_play="yes"
                            )
        ...
        if control == PlayerControls.embed_add_fav:
            ...
            try:
                ...
                await interaction.send(
                    "A embed da mensagem foi removida...", ephemeral=True
                )
                return

            if retry_after := self.bot.pool.add_fav_embed_cooldown.get_bucket(
                interaction
            ).update_rate_limit():
                await interaction.send(
            ...
            user_data = await self.bot.get_global_data(
                interaction.author.id, db_name=DBModel.users
            )

            if self.bot.config["MAX_USER_FAVS"] > 0 and not (
                await self.bot.is_owner(interaction.author)
            ):

                if (current_favs_size := len(user_data["fav_links"])) > self.bot.config[
                    "MAX_USER_FAVS"
                ]:
                    await interaction.edit_original_message(
                        f"A quantidade de itens no seu arquivo de favorito excede "
                        f"a quantidade máxima permitida ({self.bot.config['MAX_USER_FAVS']})."
                    )
                    return

                if (
                    current_favs_size + (user_favs := len(user_data["fav_links"]))
                ) > self.bot.config["MAX_USER_FAVS"]:
                    await interaction.edit_original_message(
            ...
            await self.bot.update_global_data(
                interaction.author.id, user_data, db_name=DBModel.users
            )
            ...
            try:
                interaction.message.embeds[0].fields[0].value = (
                    ...
                    .fields[0]
                    .value.replace(interaction.author.mention, "")
                )
            except IndexError:
                interaction.message.embeds[0].add_field(
                    name="**Membros que favoritaram o link:**",
            ...
            await interaction.send(
                embed=disnake.Embed(
                    ...
                    text=f"Caso queira ver todos os seus favoritos use o comando {global_data['prefix'] or self.bot.default_prefix}{self.fav_manager_legacy.name}"
                ),
                ephemeral=True,
            )
        ...
        try:
            ...
            if control == PlayerControls.add_song:
                ...
                if not interaction.user.voice:
                    raise GenericError(
                        "**You must enter a voice channel to use this button.**"
                    )

                await interaction.response.send_modal(
                    title="Ask for a song",
                    custom_id=f"modal_add_song"
                    + (
                        f"_{interaction.message.id}"
                    ...
                    components=[
                        disnake.ui.TextInput(
                            ...
                            label="Music Name/Link.",
                            placeholder="Name or Link youtube/spotify/soundcloud etc.",
                        ...
                        disnake.ui.TextInput(
                            style=disnake.TextInputStyle.short,
                            label="Position of the queue (number).",
            ...
            if control == PlayerControls.enqueue_fav:
                ...
                try:
                    player: LavalinkPlayer = self.bot.music.players[
                        interaction.guild_id
                    ]
                except KeyError:
                    await interaction.send(
                        "There is no active player on the server...", ephemeral=True
                    )
                ...
                if player.interaction_cooldown:
                    raise GenericError(
                        "The player is in Cooldown, try again in moments."
                    )
                ...
                if control == PlayerControls.help_button:
                    embed = disnake.Embed(
                        ...
                        "⏮️ `= Back to the music played earlier.`\n"
                        "⏭️ `= Jump to the next song.`\n"
                        "🔀 `= Mix the songs in the queue.`\n"
                        "🎶 `= Add music/playlist/favorito.`\n"
                ...
                if (
                    not interaction.author.voice
                    or interaction.author.voice.channel != vc
                ):
                    raise GenericError(
                        f"You must be on the channel <#{vc.id}> to use the Player buttons."
                    )
                ...
                if control == PlayerControls.add_favorite:
                    ...
                    if not player.current:
                        await interaction.send(
                            "**Não há música tocando atualmente...**", ephemeral=True
                        )
                        return
                    ...
                    if not choices:
                        try:
                            await self.player_interaction_concurrency.release(
                                interaction
                            )
                        except:
                            pass
                    ...
                    if len(choices) == 1:
                        ...
                        await interaction.send(
                            embed=disnake.Embed(
                                color=self.bot.get_color(interaction.guild.me),
                                description=f"### Selecione um item da música atual para add to your favorites:"
                                f"\n\n{msg}",
                            ),
                            view=view,
                            ephemeral=True,
                    ...
                    user_data["fav_links"][
                        fix_characters(
                            info["name"], self.bot.config["USER_FAV_MAX_URL_LENGTH"]
                        )
                    ] = info["url"]
                    ...
                    global_data = await self.bot.get_global_data(
                        interaction.author.id, db_name=DBModel.guilds
                    )
                ...
                if control == PlayerControls.lyrics:
                    if not player.current:
                        try:
                            ...
                            pass
                        await interaction.send(
                            "**Não estou tocando algo no momento...**", ephemeral=True
                        )
                        return
                    ...
                    if not player.current.ytid:
                        try:
                            ...
                            pass
                        await interaction.send(
                            "No momento apenas músicas do youtube são suportadas.",
                            ephemeral=True,
                        )
                        return
                    ...
                    if player.current.info["extra"].get("lyrics") is None:
                        player.current.info["extra"]["lyrics"] = (
                            await player.node.fetch_ytm_lyrics(player.current.ytid)
                        )
                    ...
                    player.current.info["extra"]["lyrics"]["track"]["albumArt"] = (
                        player.current.info["extra"]["lyrics"]["track"]["albumArt"][:-1]
                    )
                    ...
                    try:
                        lyrics_string = "\n".join(
                            [
                                d["line"]
                                for d in player.current.info["extra"]["lyrics"]["lines"]
                            ]
                        )
                    except KeyError:
            ...
            await self.process_player_interaction(
                interaction=interaction, command=cmd, kwargs=cmd_kwargs
            )
            ...
            try:
                ...
                if position:
                    if not position.isdigit():
                        raise GenericError(
                            "**A posição da fila tem que ser um número.**"
                        )
                    position = int(position)

                    if position < 1:
                        raise GenericError(
                            "**Número da posição da fila tem que ser 1 ou superior.**"
                        )
    ...
    async def delete_message(
        self, message: disnake.Message, delay: int = None, ignore=False
    ):
    ...
    async def song_requests(
        self, ctx: Optional[CustomContext], message: disnake.Message
    ):
        ...
        if player and isinstance(message.channel, disnake.Thread) and not player.static:
            ...
            if isinstance(text_channel, disnake.Thread):
                send_message_perm = text_channel.parent.permissions_for(
                    message.guild.me
                ).send_messages_in_threads
            else:
                send_message_perm = text_channel.permissions_for(
                    message.guild.me
                ).send_messages
            ...
            if not self.bot.intents.message_content:
                ...
                await message.channel.send(
                    ...
                    embed=disnake.Embed(
                        ...
                        "Tente add music usando **/play** ou clique em um dos botões abaixo:",
        ...
        try:
            ...
            if not message.content:
                ...
                try:
                    ...
                    await message.channel.send(
                        f"{message.author.mention} você deve enviar um link/nome da música.",
                        delete_after=8,
                    )
                    return
                    ...
                    if attachment.size > 18000000:
                        await message.channel.send(
                            f"{message.author.mention} o arquivo que você enviou deve ter o tamanho "
                            f"inferior a 18mb.",
                            delete_after=8,
                        )
                        return
            ...
            if not urls:
                ...
                if "&list=" in message.content:
                    ...
                    msg = await message.channel.send(
                        message.author.mention, embed=embed, view=view
                    )
                    ...
                    if view.selected == "music":
                        message.content = YOUTUBE_VIDEO_REG.match(
                            message.content
                        ).group()

            await self.parse_song_request(
                message, text_channel, data, response=msg, source=source
            )
            ...
            try:
                error_msg, full_error_msg, kill_process, components, mention_author = (
                    parse_error(ctx, e)
                )
            except:
                has_exception = e
            ...
            try:
                try:
                    (
                        ...
                        components,
                        mention_author,
                    ) = parse_error(message, has_exception)
                except:
                ...
                embed.set_footer(
                    ...
                    icon_url=message.author.display_avatar.with_static_format(
                        "png"
                    ).url,
                )
                ...
                embed.add_field(
                    name="Conteúdo do pedido de música:",
                    inline=False,
                    value=f"```\n{message.content}```",
                )
                ...
                await cog.send_webhook(
                    embed=embed,
                    file=string_to_file(
                        full_error_msg, "error_traceback_songrequest.txt"
                    ),
                )
    ...
    async def create_player(
        ...
        if global_data["global_skin"]:
            ...
            static_skin = (
                global_data["player_skin_static"]
                or guild_data["player_controller"]["static_skin"]
            )
        ...
        if invite is None:
            try:
                ...
                await self.bot.update_global_data(
                    inter.guild_id, global_data, db_name=DBModel.guilds
                )
        ...
        if static_player["channel"]:
            ...
            try:
                static_channel = bot.get_channel(
                    int(static_player["channel"])
                ) or await bot.fetch_channel(int(static_player["channel"]))
            except disnake.Forbidden:
                pass
        ...
        if not player.message and player.text_channel:
            try:
                player.message = await player.text_channel.fetch_message(
                    int(static_player["message_id"])
                )
            except TypeError:
                player.message = None
            except Exception:
                traceback.print_exc()
                if (
                    hasattr(player.text_channel, "parent")
                    and isinstance(player.text_channel.parent, disnake.ForumChannel)
                    and str(player.text_channel.id) == static_player["message_id"]
                ):
                    pass
                elif player.static:
        ...
        if not player.static and player.text_channel:
            ...
            if not player.has_thread:
                ...
                await self.thread_song_request(
                    message_inter.thread, reopen=True, bot=bot
                )

        return player

    async def parse_song_request(
        self,
        ...
        *,
        ...
        if not message.author.voice:
            raise GenericError(
                "Você deve entrar em um voice channel para pedir uma música."
            )
        ...
        tracks, node = await self.get_tracks(
            message.content, message.author, source=source
        )
        tracks = await self.check_player_queue(
            message.author, self.bot, message.guild.id, tracks
        )
        ...
        if not player.message:
            try:
                ...
                cached_message = await send_idle_embed(
                    message, bot=self.bot, guild_data=data
                )
                data["player_controller"]["message_id"] = str(cached_message.id)
                await self.bot.update_data(
                    message.guild.id, data, db_name=DBModel.guilds
                )
        ...
        try:
            components = [
                disnake.ui.Button(
                    emoji="🎛️", label="Player-controller", url=player.message.jump_url
                )
            ]
        except AttributeError:
            components = []
        ...
        if not isinstance(tracks, list):
            ...
            if isinstance(message.channel, disnake.Thread) and not isinstance(
                message.channel.parent, disnake.ForumChannel
            ):
                ...
                embed.set_author(
                    name="⠂" + fix_characters(tracks.tracks[0].playlist_name, 35),
                    url=message.content,
                    icon_url=music_source_image(tracks.tracks[0].info["sourceName"]),
                )
                ...
                if response:
                    ...
                    await message.reply(
                        embed=embed, fail_if_not_exists=False, mention_author=False
                    )
            ...
            if isinstance(message.channel, disnake.Thread) and not isinstance(
                ...
                try:
                    self.bot.pool.enqueue_track_embed_cooldown.get_bucket(
                        message
                    ).update_rate_limit()
                except:
                    pass
                ...
                embed.set_author(
                    ...
                    url=track.uri or track.search_uri,
                    icon_url=music_source_image(track.info["sourceName"]),
                )
                if response:
                    await response.edit(
                        content=None, embed=embed, components=components
                ...
                duration = (
                    time_format(tracks[0].duration)
                    if not tracks[0].is_stream
                    else "🔴 Livestream"
                )
                player.set_command_log(
    ...
    async def interaction_message(
        self,
        inter: Union[disnake.Interaction, CustomContext],
        txt,
        emoji: str = "✅",
        rpc_update: bool = False,
        data: dict = None,
        store_embed: bool = False,
        force=False,
        defered=False,
        thumb=None,
        components=None,
        ...
        await player.update_message(
            interaction=(
                inter
                if (bot.user.id == self.bot.user.id and component_interaction)
                else False
            ),
        ...
        if isinstance(inter, CustomContext):
            ...
            if store_embed and not player.controller_mode and len(player.queue) > 0:
                ...
                try:
                    await inter.store_message.edit(
                        embed=embed, view=None, content=None, **kwargs
                    )
                except AttributeError:
    ...
    async def node_connection_closed(self, node: wavelink.Node):
        ...
        self.bot.wavelink_node_reconnect_tasks[node.identifier] = (
            self.bot.loop.create_task(self.node_reconnect(node))
        )
    ...
    async def node_reconnect(self, node: wavelink.Node):
        ...
        if node._retry_count >= 4:
            print(
                f"❌ - {self.bot.user} - [{node.identifier} / v{node.version}] Reconexão cancelada."
            )
            node._retry_count = 0
            return
        else:
        ...
        print(
            f"⚠️ - {self.bot.user} - [{node.identifier} / v{node.version}] Conexão perdida - reconectando em {int(backoff)} segundos."
        )
        ...
        while True:
            ...
            if (
                self.bot.config["LAVALINK_RECONNECT_RETRIES"]
                and retries == self.bot.config["LAVALINK_RECONNECT_RETRIES"]
            ):
                print(
                    f"❌ - {self.bot.user} - [{node.identifier}] Todas as tentativas de reconectar falharam..."
                )
                return
            ...
            try:
                async with self.bot.session.get(
                    f"{node.rest_uri}/v4/info", timeout=45, headers=node.headers
                ) as r:
                    if r.status == 200:
                        node.version = 4
                        ...
                        raise Exception(
                            f"{self.bot.user} - [{r.status}]: {await r.text()}"[:300]
                        )
                    else:
                        node.version = 3
    ...
    async def node_ready(self, node: wavelink.Node):
        print(
            f"🌋 - {self.bot.user} - Music server: [{node.identifier} / v{node.version}] is soon for use!"
        )
        retries = 25
        while retries > 0:
            ...
            with suppress(IndexError, ValueError):
                ...
                if "deezer" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        ...
                        ["dzisrc:{isrc}", "dzsearch:{title} - {author}"],
                    )
                ...
                if "tidal" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        node.partial_providers, ["tdsearch:{title} - {author}"]
                    )
                ...
                if "applemusic" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        node.partial_providers, ["amsearch:{title} - {author}"]
                    )
                ...
                if "bandcamp" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        node.partial_providers, ["bcsearch:{title} - {author}"]
                    )
                ...
                if "spotify" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        node.partial_providers, ["spsearch:{title} - {author}"]
                    )
                ...
                if (
                    "youtube" not in node.info["sourceManagers"]
                    and "ytsearch" not in node.original_providers
                ):
                    self.remove_provider(node.search_providers, ["ytsearch"])
                    self.remove_provider(
                        node.partial_providers,
                        ['ytsearch:"{isrc}"', 'ytsearch:"{title} - {author}"'],
                    )
                ...
                if (
                    "youtube" not in node.info["sourceManagers"]
                    and "ytmsearch" not in node.original_providers
                ):
                    self.remove_provider(node.search_providers, ["ytmsearch"])
                    self.remove_provider(
                        node.partial_providers,
                        ['ytmsearch:"{isrc}"', 'ytmsearch:"{title} - {author}"'],
                    )
                ...
                if "soundcloud" not in node.info["sourceManagers"]:
                    ...
                    self.remove_provider(
                        node.partial_providers, ["scsearch:{title} - {author}"]
                    )
    ...
    async def connect_node(self, data: dict):
        ...
        data["rest_uri"] = (
            "https" if data.get("secure") else "http"
        ...
        search_providers = data.pop("search_providers", None) or [
            "ytsearch",
            "scsearch",
        ]
        ...
        node = await self.bot.music.initiate_node(
    ...
    async def get_tracks(
        ...
        if bot.pool.config["FORCE_USE_DEEZER_CLIENT"] or [
            n
            for n in bot.music.nodes.values()
            if "deezer" not in n.info.get("sourceManagers", [])
        ]:
            tracks = await self.bot.pool.deezer.get_tracks(
                url=query, requester=user.id, search=False
            )

        if not tracks:

            tracks = await self.bot.pool.spotify.get_tracks(
                self.bot, user.id, query, search=False
            )
            ...
            if not tracks:
                ...
                if not node:
                    nodes = sorted(
                        [
                            n
                            for n in bot.music.nodes.values()
                            if n.is_available and n.available
                        ],
                    ...
                    nodes = sorted(
                        [
                            n
                            for n in bot.music.nodes.values()
                            if n != node and n.is_available and n.available
                        ],
                        key=lambda n: len(n.players),
                    )
                    nodes.insert(0, node)
                ...
                for n in nodes:
                    ...
                    if source is False:
                        ...
                        if query.startswith("https://www.youtube.com/live/"):
                            ...
                            query.startswith("https://listen.tidal.com/album/")
                            and "/track/" in query
                        ):
                    ...
                    for search_provider in providers:
                        ...
                        try:
                            search_query = (
                                f"{search_provider}:{query}" if source else query
                            )
                            tracks = await n.get_tracks(
                                search_query,
                                track_cls=LavalinkTrack,
                ...
                if not tracks:
                    ...
                    try:
                        tracks = await self.bot.pool.deezer.get_tracks(
                            url=query, requester=user.id, search=True
                        ) or await self.bot.pool.spotify.get_tracks(
                            self.bot, user.id, query, search=True
                        )
                    except Exception as e:
                        exceptions.add(repr(e))
                    ...
                    if not tracks:
                        ...
                        if (
                            is_yt_source
                            and "Video returned by YouTube isn't what was requested"
                            in txt
                        ):
                        ...
                        if txt:
                            ...
                            raise GenericError(
                                f"**Ocorreu um erro ao processar sua busca:** \n{txt}",
                                error=txt,
                            )
                ...
                if isinstance(tracks, list):
                    ...
                    if (selected := tracks.data["playlistInfo"]["selectedTrack"]) > 0:
                        tracks.tracks = (
                            tracks.tracks[selected:] + tracks.tracks[:selected]
                        )

        return tracks, node

    @commands.Cog.listener("on_thread_create")
    async def thread_song_request(
        self, thread: disnake.Thread, reopen: bool = False, bot: BotCore = None
    ):
        ...
        if not thread.parent.permissions_for(thread.guild.me).send_messages_in_threads:
            await player.text_channel.send(
                embed=disnake.Embed(
                    ...
                    description="**Não tenho permissão de enviar mensagens em conversas do canal atual para activate "
        ...
        if not bot.intents.message_content:
            embed.description = (
                "**Aviso! Não estou com a intent de message_content ativada por meu desenvolvedor...\n"
            ...
            if reopen:
                ...
                embed.description = (
                    "**Essa conversa será usada temporariamente para pedido de músicas.**\n\n"
    ...
    async def player_vc_disconnect(
        ...
        if member.bot:
            ...
            if player.bot.user.id == member.id and not after.channel:
                ...
                try:
                    ...
                    pass
                player.reconnect_voice_channel_task = player.bot.loop.create_task(
                    player.reconnect_voice_channel()
                )
                ...
                if after.channel == vc:
                    ...
                    try:
                        check = (
                            m
                            for m in vc.members
                            if not m.bot and not (m.voice.deaf or m.voice.self_deaf)
                        )
                    except:
                        check = None
                    player.members_timeout_task = player.bot.loop.create_task(
                        player.members_timeout(check=bool(check))
                    )
            return
            ...
            try:
                if isinstance(before.channel, disnake.StageChannel):
                    ...
                    if before.channel.instance and member not in before.channel.members:
                        try:
                            await before.channel.instance.edit(
                                topic="atualização automática desativada"
                            )
                        except:
                            traceback.print_exc()
                        player.stage_title_event = False
                    ...
                    if (
                        isinstance(before.channel, disnake.VoiceChannel)
                        and member not in before.channel.members
                    ):
                        player.stage_title_event = False
                        if player.last_stage_title:
                            self.bot.loop.create_task(
                                player.bot.edit_voice_channel_status(
                                    status=None, channel_id=before.channel.id
                                )
                            )
            except Exception:
                traceback.print_exc()
        ...
        if (
            member.bot
            and isinstance(after.channel, disnake.StageChannel)
            and after.channel.permissions_for(member).mute_members
        ):
            ...
            if vc:
                ...
                try:
                    await player.process_rpc(
                        vc,
                        users=[member.id],
                        close=not player.guild.me.voice
                        or after.channel != player.guild.me.voice.channel,
                        wait=True,
                    )
                except AttributeError:
                    traceback.print_exc()
                ...
                await player.process_rpc(
                    vc, users=[m for m in vc.voice_states if (m != member.id)]
                )

    async def reset_controller_db(
        self, guild_id: int, data: dict, inter: disnake.AppCmdInter = None
    ):
        ...
        try:
            return sorted(
                [
                    n
                    for n in bot.music.nodes.values()
                    if n.stats and n.is_available and n.available
                ],
        ...
        while True:
            ...
            async with aiohttp.ClientSession() as session:
                webhook = disnake.Webhook.from_url(
                    self.bot.config["AUTO_ERROR_REPORT_WEBHOOK"], session=session
                )
                await webhook.send(
