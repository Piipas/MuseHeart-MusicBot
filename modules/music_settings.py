...
def check_channel_perm(
    ...
    if isinstance(channel, disnake.ForumChannel):
        missing_perms = [
            p
            for p, v in channel.permissions_for(channel.guild.me)
            if p in forum_perms and not v
        ]
    elif isinstance(channel, disnake.Thread):
        missing_perms = [
            p
            for p, v in channel.parent.permissions_for(channel.guild.me)
            if p in thread_perms and not v
        ]
    else:
        missing_perms = [
            p
            for p, v in channel.permissions_for(channel.guild.me)
            if p in channel_perms and not v
        ]
    ...
    if missing_perms:
        raise GenericError(
            ...
            + "\n".join(
                f"[0;33m{perms_translations.get(p, p)}[0m" for p in missing_perms
            )
            + "```"
        )
...
class SkinSelector(disnake.ui.View):
    ...
    def __init__(
        ...
        if not global_mode:
            ...
            self.static_skin_selected = [
                s.value for s in static_select_opts if s.default
            ][0]
        else:
            try:
                self.skin_selected = [s.value for s in global_select_opts if s.default][
                    0
                ]
            except IndexError:
            ...
            try:
                self.static_skin_selected = [
                    s.value for s in global_static_select_opts if s.default
                ][0]
            except IndexError:
    ...
    def rebuild_selects(self):
        ...
        static_select_opts = disnake.ui.Select(
            options=static_select_opts, min_values=1, max_values=1
        )
        static_select_opts.callback = self.static_skin_callback
        self.add_item(static_select_opts)

        global_mode = disnake.ui.Button(
            label=("Disable" if self.global_mode else "Activate") + " modo Global ",
            emoji="🌐",
        )
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        await inter.send(
            f"Apenas {self.ctx.author.mention} pode interagir aqui!", ephemeral=True
        )
        return False
...
class PlayerSettings(disnake.ui.View):
    ...
    def __init__(
        self, ctx: Union[disnake.AppCmdInter, CustomContext], bot: BotCore, data: dict
    ):
        super().__init__()
        self.ctx = ctx
        self.bot = bot
    ...
    def load_buttons(self):
        ...
        player_volume_select = disnake.ui.Select(
            ...
            options=[
                disnake.SelectOption(
                    label=f"Volume padrão: {i}",
                    default=i == self.default_player_volume,
                    value=str(i),
                )
        ...
        check_other_bots_button = disnake.ui.Button(
            label="Não conectar com bots incompatíveis.",
        ...
        restrict_mode_button = disnake.ui.Button(
            label="Modo restrito", emoji="✅" if self.enable_restrict_mode else "🚫"
        )
        restrict_mode_button.callback = self.restrict_mode_callback
        self.add_item(restrict_mode_button)

        check_autoplay_button = disnake.ui.Button(
            label="Autoplay.", emoji="✅" if self.enable_autoplay else "🚫"
        )
        ...
        try:
            if isinstance(self.ctx, CustomContext):
                await self.message.edit(
                    content="Alterações salvas com sucesso!", view=None, embed=None
                )
            else:
    ...
    async def save_data(self):
        ...
        await self.bot.update_data(
            self.ctx.guild_id, guild_data, db_name=DBModel.guilds
        )
    ...
    async def on_timeout(self):
        ...
        if isinstance(self.ctx, CustomContext):
            await self.message.edit(
                embed=disnake.Embed(
                    description="**Tempo esgotado...**", color=self.bot.get_color()
                ),
                view=None,
            )
        else:
...
class MusicSettings(commands.Cog):
    ...
    def __init__(self, bot: BotCore):
        ...
        self.invite_cooldown = commands.CooldownMapping.from_cooldown(
            rate=1, per=45, type=commands.BucketType.guild
        )

    player_settings_cd = commands.CooldownMapping.from_cooldown(
        1, 5, commands.BucketType.guild
    )
    player_settings_mc = commands.MaxConcurrency(
        1, per=commands.BucketType.guild, wait=False
    )
    ...
    @commands.command(
        ...
        cooldown=player_settings_cd,
        max_concurrency=player_settings_mc,
    )
    ...
    async def player_settings(self, interaction: disnake.AppCmdInter):
        ...
        view.message = await func(
            ...
            ).set_author(name=str(bot.user), icon_url=bot.user.display_avatar.url),
            view=view,
        )
    ...
    @commands.command(
        name="setup",
        aliases=["songrequestchannel", "sgrc"],
        usage="{prefix}{cmd} [id|#canal]\nEx: {prefix}{cmd} #canal",
        description="Criar/escolher um canal dedicado para pedir músicas e deixar player fixado.",
        cooldown=setup_cd,
        max_concurrency=setup_mc,
        extras={"flags": setup_args},
    )
    async def setup_legacy(
        self,
        ctx: CustomContext,
        channel: Union[
            disnake.TextChannel, disnake.VoiceChannel, disnake.ForumChannel, None
        ] = None,
        *args,
    ):
        ...
        await self.setup.callback(
            self=self, interaction=ctx, target=channel, purge_messages=args.reset
        )
    ...
    async def setup(
        ...
        if isinstance(target, disnake.ForumChannel) and not isinstance(
            inter, CustomContext
        ):
        ...
        channel_kwargs = {
            "overwrites": {guild.me: disnake.PermissionOverwrite(**perms_dict)}
        }
        ...
        if not original_message:
            ...
            try:
                ...
                original_message = await channel_db.fetch_message(
        ...
        async def get_message(original_message, target):
            ...
            if (
                original_message
                and original_message.channel != target
                and original_message.guild.id == target.guild.id
            ):

                try:
                    if isinstance(
                        original_message.channel.parent, disnake.ForumChannel
                    ):
                        await original_message.thread.delete(
                ...
                try:
                    await original_message.edit(
                        content=None, embed=embed_archived, view=None
                    )
                except:
                    pass
        ...
        if not target:
            ...
            buttons = [
                disnake.ui.Button(
                    ...
                    custom_id=f"text_channel_{id_}",
                    emoji="💬",
                    disabled=not guild.me.guild_permissions.manage_channels,
                ),
                disnake.ui.Button(
                    label="Criar voice channel",
                    custom_id=f"voice_channel_{id_}",
                    emoji="🔊",
                    disabled=not guild.me.guild_permissions.manage_channels,
                ),
                disnake.ui.Button(
                    label="Cancelar",
                    custom_id=f"voice_channel_cancel_{id_}",
                    emoji="❌",
                ),
            ]
            ...
            if "COMMUNITY" in guild.features:
                buttons.insert(
                    2,
                    disnake.ui.Button(
                        label="Criar canal de palco",
                        custom_id=f"stage_channel_{id_}",
            ...
            embeds = [
                disnake.Embed(
                    ...
                    + (
                        "ou clique em um dos botões abaixo para criar um novo canal para pedir músicas."
                    ...
                    text="Você tem apenas 45 segundos para selecionar/clicar em uma opção."
                )
            ]
            ...
            if isinstance(inter, CustomContext):
                bot_inter = bot
                check = (
                    lambda i: i.message.id == msg_select.id
                    and i.author.id == inter.author.id
                )
            else:
                bot_inter = inter.bot
                check = (
                    lambda i: i.data.custom_id.endswith(f"_{id_}")
                    and i.author.id == inter.author.id
                )

            done, pending = await asyncio.wait(
                [
                    bot_inter.loop.create_task(
                        bot_inter.wait_for("button_click", check=check)
                    ),
                    bot_inter.loop.create_task(
                        bot_inter.wait_for("dropdown", check=check)
                    ),
                ],
                timeout=45,
            ...
            if not done:
                ...
                try:
                    await func(
                        embed=disnake.Embed(
                            description="**Tempo esgotado!**", color=disnake.Color.red()
                        ),
                        components=None,
                    )
            ...
            if (
                channel.category
                and channel.category.permissions_for(guild.me).send_messages
            ):
            ...
            if inter_message.data.custom_id.startswith("existing_channel_"):
                ...
                if not guild.me.guild_permissions.manage_channels:
                    raise GenericError(
                        f"**O bot {bot.user.mention} não possui permissão de gerenciar canais pra criar um novo canal.**"
                    )

                await inter_message.response.defer()
                if inter_message.data.custom_id.startswith("voice_channel_"):
                    target = await target.create_voice_channel(
                        f"{bot.user.name} player controller", **channel_kwargs
                    )
                elif inter_message.data.custom_id.startswith("stage_channel_"):
                    target = await target.create_stage_channel(
                        f"{bot.user.name} player controller", **channel_kwargs
                    )
                else:
                    target = await target.create_text_channel(
                        f"{bot.user.name} player controller", **channel_kwargs
                    )
        ...
        if target == guild.public_updates_channel:
            raise GenericError(
                "**Você não pode usar um canal de atualizações do discord.**"
            )
        ...
        if isinstance(target, disnake.ForumChannel):
            ...
            if not inter.response.is_done():
                ...
                try:
                    modal_inter: disnake.ModalInteraction = await inter.bot.wait_for(
                        "modal_submit", timeout=30, check=lambda i: i.custom_id == id_
                    )
                except asyncio.TimeoutError:
                    try:
                        func = inter.edit_original_message
            ...
            for t in target.threads:
                if t.owner_id == bot.user.id:
                    ...
                    if (
                        thread.locked
                        and target.permissions_for(target.guild.me).manage_threads
                    ):
            ...
            message = await send_idle_embed(
                target=message, bot=bot, force=True, guild_data=guild_data
            )
            ...
            if (
                existing_channel
                and not guild.me.guild_permissions.administrator
                and not target.permissions_for(guild.me).manage_permissions
            ):
                raise GenericError(
                    f"**{guild.me.mention} não possui permissão de administrador ou permissão de "
            ...
            if not target.permissions_for(guild.me).read_messages:
                raise GenericError(
                    f"{bot.user.mention} permissão para ler mensagens no canal {target.mention}"
                )

            if purge_messages == "yes":
                await target.purge(
                    limit=100, check=lambda m: m.author != guild.me or not m.thread
                )
        ...
        if player and player.text_channel != target:
            ...
            if not message or message.channel.id != channel.id:
                message = await send_idle_embed(
                    channel, bot=bot, force=True, guild_data=guild_data
                )
            ...
            if not message.thread:
                if channel.permissions_for(guild.me).create_public_threads:
                    await message.create_thread(
                        name="Song-Requests", auto_archive_duration=10080
                    )
            else:
                thread_kw = {}
                if (
                    message.thread.locked
                    and message.thread.permissions_for(guild.me).manage_threads
                ):
                    thread_kw.update({"locked": False, "archived": False})
                ...
                if thread_kw:
                    await message.thread.edit(
                        reason=f"Song request reativado por: {inter.author}.",
                        **thread_kw,
                    )
        elif (
            player
            and isinstance(channel, (disnake.VoiceChannel, disnake.StageChannel))
            and player.guild.me.voice.channel != channel
        ):
        ...
        reset_txt = (
            f"{inter.prefix}reset" if isinstance(inter, CustomContext) else "/reset"
        )
    ...
    async def reset_legacy(self, ctx: CustomContext, *, delete_channel: str = None):
        ...
        await self.reset.callback(
            self=self, interaction=ctx, delete_channel=delete_channel
        )
    ...
    async def reset(
        ...
        delete_channel: str = commands.Param(
            name="deletar_canal",
            description="deletar o canal do player controller",
            default=None,
            choices=["sim", "não"],
        ),
    ):
        ...
        if not guild.me.guild_permissions.manage_threads:
            raise GenericError(
                f"Não tenho permissão de **{perms_translations['manage_threads']}** no servidor."
            )
        ...
        if not channel or channel.guild.id != inter.guild_id:
            raise GenericError(
                f"**Não há canais de pedido de música configurado (ou o canal foi deletado).**"
            )
            ...
            if isinstance(channel.parent, disnake.ForumChannel):
                ...
                if channel_inter != channel:
                    await inter.edit_original_message(
                        "O post foi deletado com sucesso!", embed=None, components=None
                    )
        ...
        guild_data["player_controller"].update({"message_id": None, "channel": None})
        ...
        try:
            if delete_channel == "sim":
                ...
                await original_message.thread.edit(
                    archived=True, reason=f"Player resetado por {inter.author}."
                )
    ...
    @commands.command(
        name="adddjrole",
        description="Adicionar um cargo para a lista de DJ's do servidor.",
        usage="{prefix}{cmd} [id|nome|@cargo]\nEx: {prefix}{cmd} @cargo",
        cooldown=djrole_cd,
        max_concurrency=djrole_mc,
    )
    ...
    async def add_dj_role(
        ...
        if str(role.id) in guild_data["djroles"]:
            await inter.send(
                f"O cargo {role.mention} já está na lista de DJ's", ephemeral=True
            )
            return
        ...
        await inter.send(
            f"O cargo {role.mention} foi adicionado à lista de DJ's.", ephemeral=True
        )

    @commands.has_guild_permissions(manage_guild=True)
    @commands.command(
        name="removedjrole",
        description="Remover um cargo da lista de DJ's do servidor.",
        usage="{prefix}{cmd} [id|nome|@cargo]\nEx: {prefix}{cmd} @cargo",
        cooldown=djrole_cd,
        max_concurrency=djrole_mc,
    )
    ...
    async def remove_dj_role(
        ...
        if str(role.id) not in guild_data["djroles"]:
            await inter.send(
                f"O cargo {role.mention} não está na lista de DJ's\n\n"
                + "Cargos:\n"
                + " ".join(f"<#{r}>" for r in guild_data["djroles"]),
                ephemeral=True,
            )
            return
        ...
        await inter.send(
            f"O cargo {role.mention} foi removido da lista de DJ's.", ephemeral=True
        )
    ...
    @commands.has_guild_permissions(manage_guild=True)
    @commands.command(
        description="Alterar aparência/skin do player.",
        name="changeskin",
        aliases=["skin", "skins"],
        cooldown=skin_cd,
        max_concurrency=skin_mc,
    )
    ...
    async def change_skin(self, interaction: disnake.AppCmdInter):
        ...
        skin_list = [
            s for s in bot.player_skins if s not in bot.config["IGNORE_SKINS"].split()
        ]
        static_skin_list = [
            s
            for s in bot.player_static_skins
            if s not in bot.config["IGNORE_STATIC_SKINS"].split()
        ]
        ...
        static_selected = (
            guild_data["player_controller"]["static_skin"]
            or bot.pool.default_static_skin
        )

        global_selected = global_data["player_skin"] or bot.pool.default_skin
        global_static_selected = (
            global_data["player_skin_static"] or bot.pool.default_static_skin
        )

        skins_opts = [
            disnake.SelectOption(
                emoji="💠" if s.startswith("> custom_skin: ") else "🎨",
                label=f"Modo normal: {s.replace('> custom_skin: ', '')}",
                value=s,
                **(
                    {"default": True, "description": "skin atual"}
                    if selected == s
                    else {}
                ),
            )
            for s in skin_list + add_skin_prefix(global_data["custom_skins"])
        ]
        static_skins_opts = [
            disnake.SelectOption(
                emoji="💠" if s.startswith("> custom_skin: ") else "🎨",
                label=f"Song-Request: {s.replace('> custom_skin: ', '')}",
                value=s,
                **(
                    {"default": True, "description": "skin atual"}
                    if static_selected == s
                    else {}
                ),
            )
            for s in static_skin_list
            + add_skin_prefix(global_data["custom_skins_static"])
        ]
        ...
        embed = disnake.Embed(
            ...
            + "\n\n"
            "```ansi\n[33;1mModo Fixo (Song-Request):[0m``` "
            + ", ".join(
                f"[`[{s}]`]({bot.player_static_skins[s].preview})"
                for s in static_skin_list
            )
            + "\n\n`Nota: No modo global a skin será aplicada globalmente em todos os bots.`",
            colour=bot.get_color(guild.me),
        ).set_image(
            "https://cdn.discordapp.com/attachments/554468640942981147/1082887587770937455/rainbow_bar2.gif"
        )
        ...
        msg = await func(embed=embed, view=select_view)
        ...
        if select_view.global_mode:
            ...
            guild_data["player_controller"][
                "static_skin"
            ] = select_view.static_skin_selected
        ...
        if global_mode != select_view.global_mode:
            changed_skins_txt += (
                "Skin Global: `"
                + ("Ativado" if select_view.global_mode else "Desativado")
                + "`\n"
            )
        ...
        kwargs = {
            "embed": disnake.Embed(
                description=txt, color=bot.get_color(guild.me)
            ).set_footer(
                text=f"{bot.user} - [{bot.user.id}]",
                icon_url=bot.user.display_avatar.with_format("png").url,
            )
        }
        ...
        for b in self.bot.pool.get_guild_bots(interaction.guild_id):
            ...
            player.set_command_log(
                text=f"{inter.author.mention} alterou a skin do player.", emoji="🎨"
            )

            try:
                if player.controller_mode and not [
    ...
    @pool_command(
        aliases=["la"],
        description="Activate o envio de invite para ouvir junto via RPC "
    ...
    async def listenalong(self, ctx: CustomContext):
        ...
        await ctx.reply(
            ...
            .set_image(
                url="https://cdn.discordapp.com/attachments/554468640942981147/1108943648508366868/image.png"
            )
            .set_footer(
                text="Nota: crie um convite sem limitações como: datas para expirar, quantidade de usos ou "
                "apenas para um usuário usar."
            ),
            components=[
                disnake.ui.Button(
                    label="Enviar convite", custom_id=f"listen_along_{ctx.author.id}"
                )
            ],
    ...
    async def send_listen_along_invite(self, inter: disnake.MessageInteraction):
        ...
        if not inter.data.custom_id.endswith(str(inter.author.id)):
            return await inter.send(
                "**Você não pode usar este botão.**", ephemeral=True
            )

        if not inter.author.voice.channel:
            return await inter.send(
                "**Você precisa estar em um voice channel para enviar o convite.**",
                ephemeral=True,
            )
    ...
    async def listen_along_modal(self, inter: disnake.ModalInteraction):
        ...
        if retry_after:
            return await inter.send(
                "**Você deve aguardar {} para enviar o convite**".format(
        ...
        try:
            invite = await self.bot.fetch_invite(
                inter.text_values["invite_url"].strip(), with_expiration=True
            )
        except disnake.NotFound:
            return await inter.edit_original_message(
                "Link inválido ou o convite não existe/expirou"
            )

        if invite.max_uses:
            return await inter.edit_original_message(
                "O convite pode ter quantidade máxima de usos"
            )

        if invite.target_user:
            return await inter.edit_original_message(
                "O convite não pode ser configurado para apenas 1 usuário usar."
            )
        ...
        for bot in self.bot.pool.get_guild_bots(inter.guild_id):
            ...
            if not isinstance(channel, disnake.VoiceChannel):
                return await inter.edit_original_message(
                    "**Esse recurso funciona apenas em canais de voz.**"
                )
        ...
        if not channel:
            return await inter.edit_original_message(
                "**Não há bots compatíveis adicionado no servidor do invite informado.**"
            )
        ...
        if len(global_data["listen_along_invites"]) > 4:
            return await inter.edit_original_message(
                embed=disnake.Embed(
                    ...
                    + ", ".join(
                        f"[31;1m{c}[0m" for c in global_data["listen_along_invites"]
                    )
                    + "```",
                    color=self.bot.get_color(),
                )
    ...
    async def rpc_create_modal(self, inter: disnake.ModalInteraction):
        ...
        await self.bot.update_global_data(
            id_=inter.author.id, data=data, db_name=DBModel.users
        )

        await inter.edit_original_message(
            f"O seu token foi importado/editado com sucesso!\n"
    ...
    customskin_cd = commands.CooldownMapping.from_cooldown(
        1, 10, commands.BucketType.guild
    )
    customskin__mc = commands.MaxConcurrency(
        1, per=commands.BucketType.guild, wait=False
    )
    ...
    @commands.command(
        name="customskin",
        aliases=["setskin", "cskin", "cs", "ss"],
        description="Criar suas próprias skins/templates para usar no player de música.",
        cooldown=customskin_cd,
        max_concurrency=customskin__mc,
    )
    ...
    async def custom_skin(self, inter: disnake.AppCmdInter):
        ...
        view = SkinEditorMenu(
            inter, bot, guild=bot.get_guild(inter.guild_id), global_data=global_data
        )
    ...
    async def editor_placeholders(self, inter: disnake.MessageInteraction):
        ...
        await inter.send(
            ...
            embed=disnake.Embed(
                ...
                "[34;1m{player.loop.mode}[0m -> Modo de repetition do player\n"
...
class RPCCog(commands.Cog):
    ...
    @commands.command(
        description="Activate/Disable o sistema de rich-presence no seu status.",
        name="richpresence",
        aliases=["rich_presence", "rpc"],
        cooldown=rpc_cd,
    )
    ...
    @commands.slash_command(
        description=f"{desc_prefix}Activate/Disable o sistema de rich-presence no seu status.",
        cooldown=rpc_cd,
        dm_permission=False,
    )
    async def rich_presence(self, inter: disnake.AppCmdInter):

        if not self.bot.config["ENABLE_RPC_COMMAND"] and not any(
            [
                await b.is_owner(inter.author)
                for b in self.bot.pool.get_guild_bots(inter.guild_id)
            ]
        ):
            raise GenericError(
                "**Este comando está desativado nas minhas configurações...**\n"
                "Apenas o meu desenvolvedor pode activate este comando publicamente."
            )

        if not self.bot.config["RPC_PUBLIC_URL"] and not self.bot.config["RPC_SERVER"]:
            raise GenericError(
                "**O RPC_SERVER não foi configurado na ENV/ENVIRONMENTS (ou arquivo .env)**"
            )
        ...
        embed.set_footer(
            text="Nota: No momento funciona apenas no windows com discord desktop, não funciona no mobile "
            "ou discord web."
        )
        ...
        if self.bot.config["ENABLE_RPC_AUTH"]:
            ...
            components.extend(
                [
                    disnake.ui.Button(
                        label="Criar/Resetar token",
                        custom_id=f"rpc_gen.{inter.author.id}",
                        emoji="🔑",
                        row=0,
                    ),
                    disnake.ui.Button(
                        label="Importar/Editar/Ver token",
                        custom_id=f"rpc_create.{inter.author.id}",
                        emoji="✍️",
                        row=0,
                    ),
                    disnake.ui.Button(
                        label="Remover token (Disable)",
                        custom_id=f"rpc_remove.{inter.author.id}",
                        emoji="♻️",
                        row=1,
                    ),
                ]
        ...
        if isinstance(inter, CustomContext):
            components.append(
                disnake.ui.Button(
                    label="Fechar",
                    custom_id=f"rpc_close.{inter.author.id}",
                    emoji="❌",
                    row=1,
                ),
            )
    ...
    async def rpc_button_event(self, inter: disnake.MessageInteraction):
        ...
        if user_id != str(inter.author.id):
            await inter.send(
                f"Apenas <@{user_id}> pode usar os botões da mensagem!", ephemeral=True
            )
            return
        ...
        if button_id == "rpc_gen":
            ...
            await self.bot.update_global_data(
                id_=user_id, data=data, db_name=DBModel.users
            )
            msg = (
                f"O token para usar no app de RPC (Rich Presence) foi gerado com sucesso!\n\n"
            ...
            try:
                ...
                data = await self.bot.get_global_data(
                    id_=user_id, db_name=DBModel.users
                )
            ...
            msg = (
                "O token foi removido com sucesso!\n"
    ...
    async def close_presence(
        self, inter: Union[disnake.MessageInteraction, disnake.ModalInteraction]
    ):
