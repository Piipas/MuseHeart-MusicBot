    ...
    def __init__(self, ctx, session_key: str):
        ...
        if session_key:
            btn2 = disnake.ui.Button(
                label="Desvincular conta do last.fm", style=disnake.ButtonStyle.red
            )
    ...
    async def send_authurl_callback(self, interaction: disnake.MessageInteraction):
        ...
        if not self.auth_url:
            ...
            self.last_timestamp = int(
                (disnake.utils.utcnow() + datetime.timedelta(minutes=5)).timestamp()
            )

        await interaction.send(
            f'### [Clique aqui](<{self.auth_url}>) para vincular sua conta do last.fm (na página clique em "allow")\n\n'
...
class LastFmCog(commands.Cog):
    ...
    lastfm_cd = commands.CooldownMapping.from_cooldown(
        1, 45, commands.BucketType.member
    )
    ...
    async def lastfm(self, inter: disnake.AppCmdInter):
        ...
        try:
            if not inter.permissions.embed_links:
                raise GenericError(
                    f"**Você não possui permissão de inserir links/anexos no canal <#{inter.channel_id}>**"
                )
            ...
            try:
                ...
                if e.code == 9:
                    ...
                    await self.bot.update_global_data(
                        inter.author.id, data, db_name=DBModel.users
                    )
                else:
                    raise e
        ...
        if lastfm_user:
            ...
            txt = (
                f"> `👤` **⠂Usuário:** [`{name}`](<{lastfm_user['url']}>)\n"
            ...
            txt += (
                f"\n`Ouça suas músicas em um voice channel usando o comando` {slashcmd} `para registra-las na sua "
                f"conta do last.fm`\n"
            )

            embeds = [
                disnake.Embed(description=txt, color=self.bot.get_color())
                .set_thumbnail(url=lastfm_user["image"][-1]["#text"])
                .set_author(
            ...
            top_tracks = await self.bot.last_fm.user_top_tracks(
                data["lastfm"]["username"], limit=3
            )
            ...
            if top_tracks:
                ...
                embed = disnake.Embed(
                    description="\n".join(
                        f"> ` {n+1}º ` [`{t['name']}`]({t['url']}) `De:` [`{t['artist']['name']}`]({t['artist']['url']}) `(x{int(t['playcount']):,})`"
                        for n, t in enumerate(top_tracks)
                    ),
                    color=embed_color,
                ).set_author(
                    name=f"Top 3: Músicas que você mais ouviu (do total de {int(lastfm_user['track_count']):,}):",
            ...
            top_artists = await self.bot.last_fm.user_top_artists(
                data["lastfm"]["username"], limit=3
            )
            ...
            if top_artists:
                ...
                embed = disnake.Embed(
                    description="\n".join(
                        f"> ` {n+1}º ` [`{t['name']}`]({t['url']}) `(x{int(t['playcount']):,})`"
                        for n, t in enumerate(top_artists)
                    ),
                    color=embed_color,
                ).set_author(
                    name=f"Top 3: Artistas que você mais ouviu (do total de {int(lastfm_user['artist_count']):,}):",
            ...
            top_albuns = await self.bot.last_fm.user_top_albums(
                data["lastfm"]["username"], limit=3
            )
            ...
            if top_albuns:
                ...
                embed = disnake.Embed(
                    description="\n".join(
                        f"> ` {n+1}º ` [`{b['name']}`]({b['url']}) `de:` [`{b['artist']['name']}`]({b['artist']['url']}) `(x{int(b['playcount']):,})`"
                        for n, b in enumerate(top_albuns)
                    ),
                    color=embed_color,
                ).set_author(
                    name=f"Top 5: Álbuns que você mais ouviu (do total de {int(lastfm_user['album_count']):,}):",
            ...
            embeds = [
                ...
                .set_thumbnail(
                    url="https://www.last.fm/static/images/lastfm_avatar_twitter.52a5d69a85ac.png"
                )
                .set_footer(
                    text="Nota: No momento o registro de músicas será ignorado quando você tiver ouvindo "
        ...
        await self.bot.update_global_data(
            inter.author.id, data=data, db_name=DBModel.users
        )
        ...
        if view.session_key:
            embeds[0].description += (
                ...
                "vinculada com sucesso!\n\n`Agora ao ouvir suas músicas no voice channel elas "
            ...
            for b in self.bot.pool.get_all_bots():
                for p in b.music.players.values():
                    try:
                        if (
                            inter.guild_id == p.guild_id
                            and inter.author.id in p.last_channel.voice_states
                        ):
    ...
    async def connect_vc_update(
        ...
        try:
            ...
            if (
                fm_user["last_url"] == player.current.uri
                and fm_user["last_timestamp"]
                and datetime.datetime.utcnow() < fm_user["last_timestamp"]
            ):
                return

        await self.startscrooble(
            player=player, track=player.current or player.last_track, users=[member]
        )

    @commands.Cog.listener("on_wavelink_track_start")
    async def update_np(self, player: LavalinkPlayer):
        await self.startscrooble(
            player, track=player.current or player.last_track, update_np=True
        )

    @commands.Cog.listener("on_wavelink_track_end")
    async def startscrooble(
        self,
        ...
        if (
            not track
            or track.is_stream
            or track.duration > 480000
            or track.info["sourceName"] in ("local", "http")
        ):
            return
        ...
        if not update_np:
            ...
            if (disnake.utils.utcnow() - player.start_time).total_seconds() < (
                (track.duration * 0.75) / 1000
            ):
                return
        ...
        if not (album := track.album_name) and track.info["sourceName"] in (
            "youtube",
            "soundcloud",
        ):
            ...
            if track.ytid:
                ...
                if (
                    ...
                    if (fmdata := self.bot.last_fm.cache.get(track_query)) is None:
                        ...
                        if not result:
                            print(
                                f"⚠️ - Last.FM Scrobble - Sem resultados para a música: {track_query}"
                            )
        ...
        if (
            not album
            and not track.autoplay
            and track.info["sourceName"] in ("spotify", "deezer", "applemusic", "tidal")
        ):
        ...
        for fminfo in users_fminfo:
            ...
            try:
                ...
                if isinstance(e, LastFmException):
                    print(
                        f"last.fm failed! user: {fminfo['user_id']} - code: {e.code} - message: {e.message}"
                    )
                    if e.code == 9:
                        user_data = await self.bot.get_global_data(
                            fminfo["user_id"], db_name=DBModel.users
                        )
                        user_data["lastfm"]["sessionkey"] = ""
                        await self.bot.update_global_data(
                            fminfo["user_id"], user_data, db_name=DBModel.users
                        )
                        try:
