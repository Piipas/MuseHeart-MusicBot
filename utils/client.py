...
class BotPool:
    ...
    add_fav_embed_cooldown = commands.CooldownMapping.from_cooldown(
        rate=1, per=15, type=commands.BucketType.user
    )

    enqueue_track_embed_cooldown = commands.CooldownMapping.from_cooldown(
        rate=2, per=9, type=commands.BucketType.user
    )

    enqueue_playlist_embed_cooldown = commands.CooldownMapping.from_cooldown(
        rate=1, per=14, type=commands.BucketType.user
    )
    ...
    def load_playlist_cache(self):
        ...
        return TTLCache(
            maxsize=self.config["PLAYLIST_CACHE_SIZE"],
            ttl=self.config["PLAYLIST_CACHE_TTL"],
        )
        ...
        try:
            self.lavalink_instance = await loop.run_in_executor(
                ...
                lambda: run_lavalink(
                    ...
                    lavalink_additional_sleep=int(
                        self.config["LAVALINK_ADDITIONAL_SLEEP"]
                    ),
    ...
    async def start_bot(self, bot: BotCore):
        ...
        if e:
            ...
            if isinstance(e, disnake.PrivilegedIntentsRequired):
                e = (
                    "You have not enabled Privileged Intents in your application<br>"
                    "Access the Discord Developer Portal:<br>"
                    "https://discord.com/developers/applications/<br>"
                    'and click on your application and then click the menu "bot"<br>'
                    "And then activate all the intentions.<br>"
                    "Example Print: https://i.imgur.com/a9c1DHT.png<br>"
                    "After correcting, restart the application."
                )

                print(
                    ("=" * 30)
                    + f"\nFailed to start the bot configured in the: {bot.identifier}\n"
                    + e.replace("<br>", "\n")
                    + "\n"
                    + ("=" * 30)
                )
                ...
                e = (
                    "An invalid token was used.<br>"
                    "Review if the informed token is correct<br>"
    ...
    async def check_node(self, data: dict, loop=None):
        ...
        print(
            f"📶 - Checking if the music server [{data['identifier']}] está disponível."
        )

        while True:
            if retries >= max_retries:
                print(
                    f"❌ - All attempts to check the server [{data['identifier']}] falharam. Causa: {repr(exception)}"
                )
                return
            else:
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(
                            f"{data['rest_uri']}/v4/info", timeout=45, headers=headers
                        ) as r:
                            if r.status == 200:
                                ...
                                print(
                                    f"❌ - Lavalink Server [{data['identifier']}] - Senha incorreta!"
                                )
                                return
                            elif r.status != 404:
                                raise Exception(
                                    f"❌ - [{r.status}]: {await r.text()}"[:300]
                                )
                            else:
                                data["info"] = {
                                    "check_version": 3,
                    ...
                    if data["identifier"] != "LOCAL":
                        print(
                            f'⚠️ - Failure when checking the server [{data["identifier"]}], '
                            f"new attempt [{retries}/{max_retries}] in {backoff} second."
                        )
                    backoff += 2
                    retries += 1
    ...
    def node_check(self, lavalink_servers: dict, start_local=True, loop=None):
        ...
        if start_local and "LOCAL" not in lavalink_servers:
            localnode = {
                ...
                "retries": 120,
                "retry_403": True,
                "search_providers": self.config["SEARCH_PROVIDERS"].strip().split()
                or ["amsearch", "tdsearch", "spsearch", "ytsearch", "scsearch"],
            }
        ...
        for info in data:
            ...
            if info.get("is_partial"):
                ...
                track = LavalinkTrack(
                    ...
                    info=info,
                    playlist=playlist,
                    requester=info["extra"]["requester"],
    ...
    async def connect_rpc_ws(self):
        ...
        if not self.config["RUN_RPC_SERVER"] and (
            ...
            or self.config["RPC_SERVER"].replace(
                "$PORT", port := os.environ.get("PORT", "80")
            )
        ...
        for skin in os.listdir("./utils/music/skins/normal_player"):
            ...
            try:
                ...
                if not hasattr(skin_file, "load"):
                    print(
                        f"Skin ignorada: {skin}.py | Função load() não configurada/encontrada..."
                    )
                    continue
    ...
    def setup(self):
        ...
        if self.config["AUTO_DOWNLOAD_LAVALINK_SERVERLIST"]:
            ...
            try:
                ...
                try:
                    r = requests.get(
                        self.config["LAVALINK_SERVER_LIST"], allow_redirects=False
                    )
        ...
        if ini_file:
            ...
            try:
                ...
                for key, value in {
                    ...
                    value["port"] = value["port"].replace(
                        "{SERVER_PORT}", os.environ.get("SERVER_PORT") or "8090"
                    )
                    value["search"] = value.get("search", "").lower() != "false"
                    value["retry_403"] = value.get("retry_403", "").lower() == "true"
                    value["search_providers"] = (
                        value.get("search_providers", "").strip().split()
                    )
        ...
        if (
            os.environ.get("HOSTNAME", "").lower() == "squarecloud.app"
            and self.config.get("SQUARECLOUD_LAVALINK_AUTO_CONFIG", "").lower()
            != "false"
        ):
            for f in ("squarecloud.config", "squarecloud.app"):
                try:
                    ...
                    try:
                        ...
                        self.config["AUTO_DOWNLOAD_LAVALINK_SERVERLIST"] = (
                            not start_local
                        )
                        ...
                        print(
                            "Using auto-configuration in squarecloud\n"
        ...
        if mongo_key:
            self.mongo_database = MongoDatabase(
                mongo_key,
                timeout=self.config["MONGO_TIMEOUT"],
            ...
            print(
                "🎲 - Database in use: TinyMongo | Nota: The database files will be saved locally in the folder: local_database"
            )
        ...
        if self.config["LASTFM_KEY"] and self.config["LASTFM_SECRET"]:
            self.last_fm = LastFM(
                api_key=self.config["LASTFM_KEY"],
        ...
        def load_bot(
            ...
            if not token:
                print(f"{bot_name} Ignored (token not provided)...")
                return
            ...
            async def check(inter: disnake.ApplicationCommandInteraction):
                ...
                if (
                    inter.bot.exclusive_guild_id
                    and inter.guild_id != inter.bot.exclusive_guild_id
                ):
                    raise GenericError(
                        "This server is not authorized to use my commands..."
                    )

                if (
                    self.config["COMMAND_LOG"]
                    and inter.guild
                    and not (await inter.bot.is_owner(inter.author))
                ):
                    try:
                        print(
                ...
                if not inter.guild_id:
                    ...
                    raise GenericError(
                        "This command cannot be executed in private messages.\n"
                        "Use on any server where compatible bot is added."
                    )
                ...
                if not allow_private and not isinstance(inter.guild, disnake.Guild):
                    ...
                    for bot in inter.bot.pool.get_guild_bots(inter.guild_id):
                        if (
                            bot.appinfo
                            and (
                                bot.appinfo.bot_public
                                or await bot.is_owner(inter.author)
                            )
                            and bot.get_guild(inter.guild_id)
                        ):
                            available_bot = True
                            break
            ...
            for guild_id, guildbotsdata in guild_bots.items():
                for n, guildbottoken in enumerate(guildbotsdata):
                    load_bot(
                        ...
                        guildbottoken,
                        guild_id,
                        load_modules_log=load_modules_log,
                    )
                    load_modules_log = False
        ...
        if not self.bots:
            ...
            message = "The bot token was not configured properly!\n\n"
            ...
            if os.environ.get("REPL_SLUG"):
                ...
                print(
                    message
                    + ": How to setup guide: https://gist.github.com/zRitsu/70737984cbe163f890dae05a80a3ddbe#2---com-o-projeto-j%C3%A1-criado-prossiga-as-etapas-abaixo"
                )

                message += f'. <a href="https://gist.github.com/zRitsu/70737984cbe163f890dae05a80a3ddbe#2---com-o-projeto-j%C3%A1-criado-prossiga-as-etapas-abaixo" target="_blank">Click here</a> to see the guide on how to set up.'
        ...
        if self.config["RUN_RPC_SERVER"]:
            ...
            self.playlist_cache_updater_task = loop.create_task(
                self.playlist_cache_updater()
            )
            ...
            try:
                loop.run_until_complete(self.run_bots(self.get_all_bots()))
            except KeyboardInterrupt:
                return
...
class BotCore(commands.AutoShardedBot):
    ...
    async def sync_app_commands(self, force=False):
        ...
        try:
            with open(f"./.app_commands_sync_data/{self.user.id}.json") as f:
                synced_cmds = await self.loop.run_in_executor(
                    None, lambda: json.load(f)
                )
        except FileNotFoundError:
            synced_cmds = None
        ...
        if current_cmds == synced_cmds:
            if current_cmds:
                print(f"⚠️ - {self.user} - The commands are now synchronized.")
            return
        ...
        try:
            ...
            with open(
                f"./.app_commands_sync_data/{self.user.id}.json", "w", encoding="utf-8"
            ) as f:
                await self.loop.run_in_executor(
                    None, lambda: json.dump(current_cmds, f, indent=4)
                )

            with open(
                f"./.app_commands_sync_data/{self.user.id}-old.json",
                "w",
                encoding="utf-8",
            ) as f:
                await self.loop.run_in_executor(
                    None, lambda: json.dump(synced_cmds, f, indent=4)
                )
    ...
    async def can_send_message(self, message: disnake.Message):
        ...
        if isinstance(message.channel, disnake.Thread):
            perm_check = message.channel.parent.permissions_for(
                message.guild.me
    ...
    async def on_message(self, message: disnake.Message):
        ...
        if isinstance(message.channel, disnake.StageChannel):
            ...
            if message.channel.permissions_for(message.guild.me).read_message_history:
                ...
                await message.channel.send(
                    message.author.mention, embed=embed, **kwargs
                )
            return
        ...
        if (
            not ctx.valid
            and message.content.startswith(self.user.mention)
        ...
        try:
            if isinstance(channel.parent, disnake.ForumChannel):
                ...
                if channel.owner_id in (
                    bot.user.id
                    for bot in self.pool.get_guild_bots(channel.guild.id)
                    if bot.bot_ready
                ):
                    ...
                    raise GenericError(
                        "**You cannot use prefixed commands in the current post...**\n"
                        "`Use slash (/) command here.`",
                        self_delete=True,
                    )
        except AttributeError:
            pass
    ...
    async def update_appinfo(self):
        ...
        if self.appinfo.bot_public and not self.config.get("SILENT_PUBLICBOT_WARNING"):
            ...
            warn_msg = (
                f"Attention: The bot [{self.user}] (ID: {self.user.id}) was configured in the developer portal "
                "as a public bot\n"
            ...
            if guilds:
                warn_msg += (
                    "\n\nThe bot is currently on servers where the bot owner (or team member) does not "
                    f"or do not have permission to manage server to add their own bot "
    ...
    def load_modules(self, module_list: list = None, load_modules_log=False):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(
                                f"❌- {bot_name} - Failed to load/reload module: {filename}"
                            )
        ...
        for c in self.slash_commands:
            if (desc := len(c.description)) > 100:
                raise Exception(
                    f"The description for the command {c.name} exceeded the allowed character count "
                    f"on discord (100), current quantity: {desc}"
                )
