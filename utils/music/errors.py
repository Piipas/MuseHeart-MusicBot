...
class GenericError(commands.CheckFailure):
    ...
    def __init__(
        ...
        text: str,
        *,
        self_delete: int = None,
        delete_original: Optional[int] = None,
...
class MissingVoicePerms(commands.CheckFailure):
    ...
    def __init__(
        self, voice_channel: Union[disnake.VoiceChannel, disnake.StageChannel]
    ):
...
def parse_error(
    ...
    if isinstance(error, NotDJorStaff):
        error_txt = (
            "**Você deve estar na lista de DJ ou ter a permissão de **Mover Membros** "
        ...
        error_txt = "Não tenho as seguintes permissões para executar esse comando: ```\n{}```".format(
            ", ".join(
                perms_translations.get(perm, perm) for perm in error.missing_permissions
            )
        )
        ...
        error_txt = (
            "**Você deve ter pedido a música atual ou estar na lista de DJ ou ter a permissão de "
            "**Gerenciar canais** para pular músicas.**"
        )

    elif isinstance(error, DiffVoiceChannel):
        error_txt = (
            "**Você deve estar no meu voice channel atual para usar esse comando.**"
        )
        ...
        error_txt = "**Você deve entrar em um voice channel para usar esse comando.**"

    elif isinstance(error, NoPlayer):
        try:
            error_txt = (
                f"**Não há player ativo no canal {ctx.author.voice.channel.mention}.**"
            )
        except AttributeError:
            error_txt = "**Não há player inicializado no servidor.**"

    elif (
        isinstance(error, (commands.UserInputError, commands.MissingRequiredArgument))
        and ctx.command.usage
    ):
        ...
        if remaing < 1:
            remaing = 1
        error_txt = "**Você deve aguardar {} para usar esse comando.**".format(
        ...
        if isinstance(ctx, disnake.MessageInteraction):
            error_txt = (
                "**Você não possui favorito/integração**\n\n"
            ...
            error_txt = (
                "**Você usou o comando sem incluir um nome ou link de uma músisca ou vídeo e você não possui "
        ...
        components = [
            disnake.ui.Button(
                label="Abrir o gerenciador de favoritos e integrações",
                custom_id="musicplayer_fav_manager",
                emoji="⭐",
            ),
        ]
        ...
        error_txt = (
            f"{ctx.author.mention} **{txt[error.per]} e ainda não teve seu{'s' if error.number > 1 else ''} "
        ...
        error_txt = (
            "O suporte a links/buscas do youtube está desativado devido a medidas reforçadas do próprio youtube "
    ...
    if isinstance(error, ServerSelectionTimeoutError) and os.environ.get("REPL_SLUG"):
        error_txt = (
            "Foi detectado um erro de dns na repl.it que me impede de conectar com minha database "
        ...
        if "Unknown file format" in (wave_error := str(error)):
            ...
            error_txt = (
                "**Oo link da playlist contém um parâmetro/id não suportado...**"
            )
        elif "The playlist does not exist" in wave_error:
            error_txt = "**A playlist não existe (ou está privada).**"
        elif (
            "not made this video available in your country" in wave_error.lower()
            or "who has blocked it in your country on copyright grounds"
            in wave_error.lower()
        ):
