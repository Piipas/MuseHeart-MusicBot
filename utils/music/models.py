        ...
        difference = (
            (
                (
                    ...
                    + datetime.timedelta(milliseconds=int(extra_milliseconds))
                ).timestamp()
                * 1000
            )
        ) - player.last_update
...
class LavalinkPlaylist:
    ...
    def __init__(self, data: dict, **kwargs):
        ...
        self.tracks = [
            LavalinkTrack(
                id_=track[encoded_name],
                info=track["info"],
                pluginInfo=track.get("pluginInfo") or pluginInfo,
                thumb=thumb,
                playlist=playlist,
                **kwargs,
            )
...
class LavalinkTrack(wavelink.Track):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            if (
                albumname := fix_characters(self.info["pluginInfo"]["albumName"])
            ) == self.title:
        ...
        if self.info["sourceName"] == "youtube":
            self.info["artworkUrl"] = (
                f"https://img.youtube.com/vi/{self.ytid}/mqdefault.jpg"
            )
            ...
            self.info["artworkUrl"] = self.info.get("artworkUrl", "").replace(
                "-large.", "-t500x500."
            )
            ...
            if "?in=" not in self.uri:
                try:
                    self.uri = (
                        f"{self.uri}?in="
                        + self.playlist_url.split("soundcloud.com/")[1]
                    )
                    self.info["uri"] = self.uri
                except:
                    pass
...
class LavalinkPlayer(wavelink.Player):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.text_channel: Union[
            disnake.TextChannel, disnake.VoiceChannel, disnake.Thread
        ] = kwargs.pop("channel")
        ...
        self.skin_static: str = (
            kwargs.pop("skin_static", None) or self.bot.pool.default_static_skin
        )
        ...
        self.ignore_np_once = (
            False  # não invocar player controller em determinadas situações
        )
        self.allowed_mentions = disnake.AllowedMentions(
            users=False, everyone=False, roles=False
        )
        self.uptime = kwargs.pop("uptime", None) or int(
            disnake.utils.utcnow().timestamp()
        )
        # activate/disable modo controller (apenas para uso em skins)
        ...
        self.stage_title_template: str = (
            kwargs.pop("stage_title_template", None)
            or "Tocando: {track.title} | {track.author}"
        )
        ...
        self.initial_hints = [
            ...
            "É possível definir o status automático no voice channel com informações sobre "
        ...
        if hint_platforms:
            self.initial_hints.append(
                "Você pode adicionar/integrar link de perfis/canais do "
                + " e ".join(hint_platforms)
                + " para tocar "
    ...
    def __str__(self) -> str:
        return (
            f"Servidor de música atual: {self.node.identifier} (v{self.node.version})"
        )
    ...
    def has_thread(self):
        ...
        return (
            self.message and self.message.thread
        )  # and not (self.message.thread.locked or self.message.thread.archived)
    ...
    async def report_error(
        self, embed: disnake.Embed, track: Union[LavalinkTrack, PartialTrack]
    ):
        ...
        if cog and cog.error_report_queue:
            ...
            embed.description += (
                f"**Data:** <t:{int(disnake.utils.utcnow().timestamp())}:F>"
            )
    ...
    async def reconnect_voice_channel(self):
        ...
        if not vc:
            ...
            msg = "O voice channel foi excluido..."
        ...
        while True:
            ...
            if self.guild.me.voice:
                if (
                    isinstance(vc, disnake.StageChannel)
            ...
            if not self._new_node_task:
                ...
                if (ping := round(self.bot.latency * 1000)) > self.bot.pool.config[
                    "VOICE_CHANNEL_LATENCY_RECONNECT"
                ]:
                    voice_msg = f"Reconectei no voice channel devido a um possível problema de instabilidade ne conexão (ping: {ping}ms)."
                elif self.keep_connected:
                    voice_msg = (
                        f"Notei uma tentativa de me desconectarem do canal <#{vc.id}>."
                    )
                else:
                    voice_msg = None
                ...
                if not voice_msg:
                    self.set_command_log(
                        text=f"O player foi finalizado por perca de conexão no canal <#{vc.id}>.",
                        emoji="⚠️",
                    )
                    await self.destroy()
                    return
    ...
    async def hook_events(self, event):
        ...
        if isinstance(event, wavelink.TrackStart):
            ...
            if self.text_channel:
                ...
                if isinstance(self.text_channel, disnake.Thread):
                    send_message_perm = self.text_channel.parent.permissions_for(
                        self.guild.me
                    ).send_messages_in_threads
                else:
                    send_message_perm = self.text_channel.permissions_for(
                        self.guild.me
                    ).send_messages
            ...
            if not self.guild.me.voice:
                try:
                    await self.bot.wait_for(
                        "voice_state_update",
                        check=lambda m, b, a: m == self.guild.me and m.voice,
                        timeout=7,
                    )
        ...
        if isinstance(event, wavelink.TrackException):
            ...
            node_info = (
                f"`{event.node.identifier}`"
                if event.node.identifier == self.node.identifier
                else f"`{self.node.identifier} | {event.node.identifier}`"
            )
            embed = disnake.Embed(
            ...
            if event.cause.startswith(
                ...
                video_not_available := event.cause.startswith(
                    (
                ...
                if video_not_available:
                    ...
                    with suppress(IndexError, ValueError):
                        ...
                        self.node.partial_providers.remove(
                    ...
                    if track.info["sourceName"] == "youtube":
                        txt = (
                            f"Devido a restrições do youtube no servidor `{self.node.identifier}`. Durante a sessão atual "
            ...
            if (
                youtube_exception := (
                    event.error == "This IP address has been blocked by YouTube (429)"
                    or
                ...
                if youtube_exception and self.node.retry_403:
                    ...
                    if not self.retries_403["last_time"] or (
                        (
                            disnake.utils.utcnow() - self.retries_403["last_time"]
                        ).total_seconds()
                        > self.bot.pool.config.get("ERROR_403_RETRIES", 7)
                    ):
                        self.retries_403 = {
                        ...
                        if self.auto_pause:
                            ...
                            await self.play(
                                ...
                                start=get_start_pos(
                                    self,
                                    track,
                                    self.bot.pool.config.get(
                                        "ERROR_403_ADDITIONAL_MILLISECONDS", 430
                                    ),
                                ),
                            )
                            await asyncio.sleep(3)
            ...
            if event.cause.startswith(
                ...
                if not hasattr(self, "retries_general_errors"):
                    self.retries_general_errors = {
                        "counter": 6,
                        "last_node": self.node.identifier,
                        "last_time": disnake.utils.utcnow(),
                    }
                ...
                if (
                    ...
                    and self.node.identifier == self.retries_general_errors["last_node"]
                    and (
                        disnake.utils.utcnow()
                        - self.retries_general_errors["last_time"]
                    ).total_seconds()
                    < 180
                ):
        ...
        if isinstance(event, wavelink.WebsocketClosed):
            ...
            if event.code in (
                ...
                if self.guild and self.guild.me.voice:
                    return
                self.set_command_log(
                    f"O player foi desligado por perca de conexão com o canal {self.last_channel.mention}..."
                )
                await self.destroy(force=True)
                return
        ...
        if isinstance(event, wavelink.TrackStuck):
            ...
            try:
                self.set_command_log(
                    text=f"A música [{fix_characters(self.current.single_title, 25)}](<{self.current.uri}>) travou.",
                    emoji="⚠️",
                )
            except:
                pass
    ...
    async def channel_cleanup(self):
        ...
        try:
            ...
            if (
                isinstance(parent, disnake.ForumChannel)
                and self.text_channel.owner_id == self.bot.user.id
                and self.text_channel.message_count > 1
            ):
                try:
                    await self.text_channel.purge(
                        check=lambda m: m.channel.id != m.id
                        and (not m.pinned or not m.is_system())
                    )
                except:
                    pass
        ...
        if self.last_message_id != self.text_channel.last_message_id:
            ...
            if isinstance(self.text_channel, disnake.Thread):
                check = (
                    lambda m: m.id != self.last_message_id
                    and not not m.pinned
                    and check_current_message(m)
                    and (
                        not m.is_system()
                        or m.type != disnake.MessageType.channel_name_change
                    )
                )
            else:
                check = (
            ...
            try:
                ...
                print(
                    f"Falha ao limpar mensagens do canal {self.text_channel} [ID: {self.text_channel.id}]:\n"
    ...
    async def connect(
        self, channel_id: int, self_mute: bool = False, self_deaf: bool = False
    ):
    ...
    def setup_features(self):
        ...
        try:
            (
                ...
                if self.static
                else self.bot.player_skins[self.skin]
            ).setup_features(self)
        except:
    ...
    def setup_hints(self):
        ...
        if self.static:
            hints.append(
                "É possível fixar músicas/playlists na mensagem do player quando tiver no modo de "
            ...
            hints.append(
                "Ao criar uma conversa/thread na mensagem do player, será ativado o modo de song-request "
        ...
        if len(self.bot.pool.get_guild_bots(self.guild.id)) > 1:
            ...
            if bots_in_guild:
                hints.append(
                    "Caso algum membro queira me usar em outro voice channel sem precisar aguardar me "
                    ...
                    f"Experimente entrar em um voice channel diferente do meu atual e use o comando "
    ...
    async def members_timeout(
        self, check: bool, force: bool = False, idle_timeout=None
    ):
        ...
        if check:
            ...
            if self.current and self.auto_pause:
                ...
                try:
                    self.set_command_log(
                        emoji="🔋",
                        text="O modo **[economia de recursos]** foi desativado.",
                    )
                    await self.resolve_track(self.current)
                    if self.current.id:
                        if (
                            self.current.info["sourceName"] == "youtube"
                            and not self.native_yt
                        ):
                            stream = self.current.is_stream
                            self.queue.appendleft(self.current)
                            self.current = None
                            await self.process_next(
                                start_position=0 if stream else self.position
                            )
                        else:
                            await self.play(
                                self.current,
                                start=0 if self.current.is_stream else self.position,
                            )
        ...
        if self.keep_connected:
            ...
            msg = (
                "**O player foi desligado por falta de membros no canal"
                + (
                    f" <#{self.guild.me.voice.channel.id}>"
                    if self.guild.me.voice
                    else ""
                )
                + "...**"
            ...
            if not self.static and not self.has_thread:
                ...
                try:
                    await self.text_channel.send(
                        embed=embed, allowed_mentions=self.allowed_mentions
                    )
                except:
                    pass
    ...
    async def get_autoqueue_tracks(self):
        ...
        if tracks_search:
            ...
            for track_data in tracks_search:
                ...
                if (
                    track_data.info["sourceName"] == "spotify"
                    and "spotify" not in self.node.info["sourceManagers"]
                    and self.bot.spotify
                ):
                    track_ids = list(
                        set(
                            t.identifier
                            for t in tracks_search
                            if t.info["sourceName"] == "spotify"
                        )
                    )[:5]
                    ...
                    for i in range(3):
                        try:
                            result = await self.bot.spotify.get_recommendations(
                                track_ids, limit=100
                            )
                            break
                        except Exception as e:
                            self.set_command_log(
                                emoji="⚠️",
                                text=f"Falha ao obter músicas recomendadas do spotify, tentativa {i+1} de 3.",
                            )
                            self.update = True
                            traceback.print_exc()
                            exception = e
                    ...
                    if result:
                        ...
                        for t in result["tracks"]:
                            ...
                            partial_track.info["extra"]["authors"] = [
                                fix_characters(i["name"])
                                for i in t["artists"]
                                if f"feat. {i['name'].lower()}" not in t["name"].lower()
                            ]
                    ...
                    track_data.info["sourceName"] == "deezer"
                    and (
                        self.bot.pool.config["FORCE_USE_DEEZER_CLIENT"]
                        or "deezer" not in self.node.info["sourceManagers"]
                    )
                    and (artist_id := track_data.info["extra"].get("artist_id"))
                ):

                    try:
                        try:
                            result = await self.bot.deezer.get_artist_radio_info(
                                artist_id
                            )
                        except Exception:
                            traceback.print_exc()
                        ...
                        if result:
                            ...
                            for n, t in enumerate(result):
                                ...
                                partial_track.info["extra"]["authors_md"] = ", ".join(
                                    f"[`{fix_characters(a['name'])}`](https://www.deezer.com/artist/{a['id']})"
                                    for a in artists
                                )
                                partial_track.info["extra"]["artist_id"] = t["artist"][
                                    "id"
                                ]
                ...
                if not tracks:
                    ...
                    if self.bot.last_fm and not self.lastfm_artists:
                        ...
                        if track_data.ytid:
                            if (
                                track_data.author.endswith(" - topic")
                                and not track_data.author.endswith("Release - topic")
                                and not track_data.title.startswith(
                                    track_data.author[:-8]
                                )
                            ):
                                ...
                                try:
                                    artist = track_data.title.split(" - ", maxsplit=1)[
                                        0
                                    ]
                                except ValueError:
                        ...
                        try:
                            self.lastfm_artists = [
                                a["name"]
                                for a in await self.bot.last_fm.get_similar_artists(
                                    artist
                                )
                                if a["name"].lower() not in track_data.author.lower()
                            ]
                    ...
                    if track_data.info["sourceName"] == "youtube" and self.native_yt:
                        queries = [
                            f"https://www.youtube.com/watch?v={track_data.ytid}&list=RD{track_data.ytid}"
                        ]

                        if p_dict := providers_dict.get(track_data.info["sourceName"]):
                            providers = [p_dict] + [
                                p for p in self.node.search_providers if p != p_dict
                            ]
                        else:
                            providers = self.node.search_providers

                        queries.extend(
                            [f"{sp}:{author.split(',')[0]}" for sp in providers]
                        )

                    elif (
                        track_data.info["sourceName"] == "spotify"
                        and "spotfy" in self.node.info["sourceManagers"]
                    ):
                        queries = [
                            "sprec:seed_tracks="
                            + ",".join(
                    ...
                    if not [i in track_data.title.lower() for i in exclude_tags_2]:
                        ...
                        for t in tracks:
                            if not any(
                                (i in t.title.lower()) for i in exclude_tags_2
                            ) and not track_data.uri.startswith(t.uri):
            ...
            if not tracks:
                ...
                if exception:
                    ...
                    error_msg = (
                        "Não houve resultados relacionados as músicas tocadas..."
                    )
        ...
        try:
            tracks = [
                t
                for t in tracks
                if not [u for u in tracks_search if t.uri.startswith(u.uri)]
            ]
        except:
            pass
        ...
        if track:
            ...
            info = {"title": track.title, "uri": track.uri}
            ...
            for t in tracks:
                ...
                if not isinstance(t, PartialTrack):
                    t.info["extra"].update(
                        {"autoplay": True, "requester": self.bot.user.id}
                    )
                    t.playlist = None
    ...
    async def process_next(
        self,
        start_position: Union[int, float] = 0,
        ...
        try:
            ...
            try:
                ...
                if not track:
                    await self.stop()
                    self.idle_endtime = disnake.utils.utcnow() + datetime.timedelta(
                        seconds=self.bot.config["IDLE_TIMEOUT"]
                    )
        ...
        if not self.auto_pause:
            ...
            if track.info["sourceName"] not in self.node.info[
                "sourceManagers"
            ] and not isinstance(track, PartialTrack):
                track.id = ""
            ...
            if not self.native_yt and (
                track.info["sourceName"] == "youtube"
                or track.info.get("sourceNameOrig") == "youtube"
            ):
                ...
                if not track.temp_id:
                    ...
                    if not (tracks := final_result):
                        ...
                        self.set_command_log(
                            emoji="⚠️",
                            text=f"A música [`{track.title[:15]}`](<{track.uri}>) será pulada devido a falta de resultado "
        ...
        if self.auto_pause:
            ...
            self.bot.loop.create_task(
                self.node.on_event(
                    TrackStart({"track": track, "player": self, "node": self.node})
                )
            )
            self.set_command_log(
                emoji="🪫",
        ...
        # TODO: rever essa parte caso adicione função de activate track loops em músicas da fila
        if (
            self.loop != "current"
            or force_np
            or (not self.controller_mode and self.current.track_loops == 0)
        ):
            ...
            await self.invoke_np(
                ...
                force=(
                    True
                    if (self.static or not self.loop or not self.is_last_message())
                    else False
                ),
                rpc_update=True,
            )
    ...
    async def process_idle_message(self):
        ...
        controller_opts = [
            ...
            disnake.SelectOption(
                ...
                description=f"Add favorite to queue.",
            ),
        ]
        ...
        components = [
            disnake.ui.Select(
                placeholder="Executar uma ação:",
                options=controller_opts,
        ...
        opts = [
            disnake.SelectOption(label=k, value=k, description=v["description"])
            for k, v in guild_data["player_controller"]["fav_links"].items()
        ]
        ...
        embed = disnake.Embed(
            description="**There are no songs in line ... add a song or use one of the options below.**",
            color=self.bot.get_color(self.guild.me),
        )

        embed.set_thumbnail(
            url=self.bot.user.display_avatar.replace(size=512, static_format="png").url
        )

        if not self.keep_connected:
            embed.description += (
                "\n\n**Use:** `The player will be turned off automatically` "
        ...
        try:
            if (
                isinstance(self.text_channel.parent, disnake.ForumChannel)
                and self.static
            ):
        ...
        try:
            if (
                self.has_thread
                or self.static
                or self.text_channel.last_message_id == self.message.id
            ):
                try:
    ...
    async def update_stage_topic(self, reconnect=True, clear=False):
        ...
        if clear:
            if (
                isinstance(self.guild.me.voice.channel, disnake.VoiceChannel)
                and self.last_stage_title
            ):
                await self.bot.edit_voice_channel_status(
                    status=None, channel_id=self.guild.me.voice.channel.id
                )
            return
        ...
        if self.current:
            ...
            if not self.current.is_stream and (not self.auto_pause or not self.paused):
                ...
                timestamp = (
                    ("pausado" if (self.paused) else "🔴")
                    + (f" <t:{int(disnake.utils.utcnow().timestamp())}:R>")
                    if not self.current.is_stream
                    else ""
                )

            msg = (
                self.stage_title_template.replace(
                    "{track.title}", self.current.single_title
                )
                .replace("{track.author}", self.current.authors_string)
                .replace(
                    "{track.duration}",
                    (
                        time_format(self.current.duration)
                        if not self.current.is_stream
                        else "Livestream"
                    ),
                )
                .replace(
                    "{track.source}",
                    self.current.info.get("sourceName", "desconhecido"),
                )
                .replace(
                    "{track.playlist}", self.current.playlist_name or "Sem playlist"
                )
        ...
        if isinstance(self.guild.me.voice.channel, disnake.StageChannel):
            ...
            if msg is not None:
                ...
                msg = msg.replace(
                    "{track.emoji}", music_source_emoji(self.current.info["sourceName"])
                )
            ...
            try:
                await self.bot.edit_voice_channel_status(
                    status=msg, channel_id=self.guild.me.voice.channel.id
                )
            except Exception as e:
                if isinstance(e, disnake.Forbidden):
                    ...
                    self.set_command_log(
                        emoji="❌",
                        text="O status automático foi desativado devido a falta de permissão pra alterar status.",
                    )
                    self.update = True
                print(traceback.format_exc())
    ...
    async def invoke_np(self, force=False, interaction=None, rpc_update=False):
        ...
        try:
            if self.static:
                if self.skin_static.startswith("> custom_skin: "):
                    data = skin_converter(
                        self.custom_skin_static_data[self.skin_static[15:]],
                        player=self,
                        guild=self.guild,
                    )
                else:
                    data = self.bot.pool.player_static_skins[self.skin_static].load(
                        self
                    )
                ...
                if self.skin.startswith("> custom_skin: "):
                    data = skin_converter(
                        self.custom_skin_data[self.skin[15:]],
                        player=self,
                        guild=self.guild,
                    )
                else:
        ...
        try:
            if self.static and isinstance(
                self.text_channel.parent, disnake.ForumChannel
            ):
                data["content"] = (
                    f"`{'▶️' if not self.paused else '⏸️'} {fix_characters(self.current.title, 50)}` |\n\n"
                    + (data.get("content") or "")
                )
        except:
            pass
        ...
        if not self.controller_mode:
            ...
            if self.keep_connected:
                if not self.is_last_message():
                    ...
                    if not self.text_channel.permissions_for(
                        self.guild.me
                    ).send_messages:
                        self.text_channel = None
                        self.message = None
                        ...
                        try:
                            await self.message.edit(
                                allowed_mentions=self.allowed_mentions, **data
                            )
                ...
                data["components"] = [
                    ...
                    disnake.ui.Select(
                        ...
                        options=[
                            ...
                            disnake.SelectOption(
                                label=("Disable" if self.nightcore else "Activate")
                            ...
                            disnake.SelectOption(
                                label=("Disable" if self.autoplay else "Activate")
                                + " autoplay",
                                emoji="🔄",
                                value=PlayerControls.autoplay,
                                description="Automatic music playback system when the line is empty.",
                            ),
                            disnake.SelectOption(
                                label=("Disable" if self.restrict_mode else "Activate")
                                + " restricted mode",
                                emoji="🔐",
                    ...
                    try:
                        ...
                        try:
                            ...
                            if isinstance(self.text_channel, disnake.Thread):
                                ...
                                if (
                                    not self.text_channel.parent.permissions_for(
                                        self.guild.me
                                    ).send_messages_in_threads
                                    or not self.text_channel.permissions_for(
                                        self.guild.me
                                    ).read_messages
                                ):
                                    return
                                ...
                                if self.text_channel.locked:
                                    ...
                                    if self.text_channel.owner_id == self.bot.user.id:
                                        ...
                                        await self.text_channel.send(
                                            "Desarquivando o tópico.", delete_after=2
                                        )
                                ...
                                not self.text_channel.permissions_for(
                                    self.guild.me
                                ).send_messages
                                or not self.text_channel.permissions_for(
                                    self.guild.me
                ...
                try:
                    ...
                    if self.static:
                        ...
                        data = await self.bot.get_data(
                            self.guild_id, db_name=DBModel.guilds
                        )
                        ...
                        await self.bot.update_data(
                            self.guild_id, data, db_name=DBModel.guilds
                        )
    ...
    async def update_message(
        self, interaction: disnake.Interaction = None, force=False, rpc_update=False
    ):
    ...
    async def cleanup(self, inter: disnake.MessageInteraction = None):
        ...
        if self.guild.me:
            ...
            self.bot.loop.create_task(
                self.update_stage_topic(reconnect=False, clear=True)
            )
            ...
            if self.static:
                ...
                try:
                    await send_idle_embed(
                        inter or self.message, self.command_log, bot=self.bot
                    )
                except:
                    traceback.print_exc()
                    pass
                    ...
                    if self.has_thread:
                        ...
                        if func:
                            try:
                                ...
                                channel: disnake.Thread = self.bot.get_channel(
                                    self.message.id
                                )

                                if channel.parent.permissions_for(
                                    self.guild.me
                                ).send_messages_in_threads:
                                    try:
                                        await channel.send(
                                ...
                                if (
                                    channel.owner.id == self.bot.user.id
                                    or channel.parent.permissions_for(
                                        self.guild.me
                                    ).manage_threads
                                ):
        ...
        try:
                ...
                if track.info["sourceName"] in self.node.info.get("sourceManagers", []):
                    ...
                    for sp in self.node.partial_providers:
                        ...
                        search_queries.append(
                            sp.replace("{title}", track.single_title).replace(
                                "{author}", ", ".join(track.authors)
                            )
                        )
            ...
            for query in search_queries:
                ...
                try:
                    ...
                    if track.info["sourceName"] == "youtube" and any(
                        e in str(e)
                        for e in (
                        ...
                        cog = self.bot.get_cog("Music")
                        cog.remove_provider(
                            self.node.search_providers, ["ytsearch", "ytmsearch"]
                        )
                        cog.remove_provider(
                            self.node.partial_providers,
                            [
                                'ytsearch:"{isrc}"',
                                'ytsearch:"{title} - {author}"',
                                'ytmsearch:"{isrc}"',
            ...
            if not tracks:
                if exceptions:
                    print(
                        "Falha ao resolver PartialTrack:\n"
            ...
            has_exclude_tags = any(
                tag for tag in exclude_tags if tag.lower() in track.title.lower()
            )
            ...
            for t in tracks:
                ...
                if not has_exclude_tags and any(
                    tag for tag in exclude_tags if tag.lower() in t.title.lower()
                ):
                    continue

                if check_duration and (
                    (t.duration - 10000) < track.duration < (t.duration + 10000)
                ):
                    selected_track = t
                    break
    ...
    async def _wait_for_new_node(
        self, txt: str = None, ignore_node: wavelink.Node = None
    ):
        ...
        try:
            ...
            self.set_command_log(
                txt
                or "Não há servidores de música disponível. Irei fazer algumas tentativas de conectar em um novo servidor de música.",
                emoji="⏰",
            )
            self.update = True
            ...
            while True:
                ...
                if not self.guild.me.voice:
                    try:
                        ...
                        print(traceback.format_exc())
                        self.set_command_log(
                            f"O player foi finalizado devido ao erro: {e}"
                        )
                        await self.destroy()
                        return
                    await self.connect(self.last_channel.id)
            ...
            if not voice_channel and not close:
                ...
                try:
                    voice_channel = self.bot.get_channel(
                        self.channel_id
                    ) or self.bot.get_channel(self.guild.voice_client.channel.id)
                except AttributeError:
            ...
            if close:
                ...
                if wait:
                    ...
                    try:
                        ...
                        pass
                    self._rpc_update_task = self.bot.loop.create_task(
                        self._send_rpc_data(users, stats)
                    )
                return
    ...
    async def process_save_queue(self, create_task=True):
        ...
        if (
            self._queue_updater_task
            and self.bot.config["PLAYER_SESSIONS_MONGODB"]
            and self.bot.config["MONGO"]
        ):
            return
        ...
        if create_task:
            self._queue_updater_task = self.bot.loop.create_task(
                cog.queue_updater_task(self)
            )
    ...
    async def destroy(
        self, *, force: bool = False, inter: disnake.MessageInteraction = None
    ):
    ...
    async def process_destroy(
        ...
        if (
            isinstance(channel, disnake.StageChannel)
            and self.stage_title_event
            and self.guild.me
            and self.guild.me.guild_permissions.manage_channels
        ):
    ...
    async def set_timescale(
        ...
        if enabled:
            return await self.set_filter(
                AudioFilter.timescale(speed=speed, pitch=pitch, rate=rate)
            )
    ...
    async def set_tremolo(
        self, frequency: float = 2.0, depth: float = 0.0, enabled: bool = True
    ):
        if enabled:
            return await self.set_filter(
                AudioFilter.tremolo(frequency=frequency, depth=depth)
            )
    ...
    async def set_vibrato(
        ...
        if enabled:
            return await self.set_filter(
                AudioFilter.vibrato(frequency=frequency, depth=depth)
            )
    ...
    async def update_filters(self):
        if self.node.version == 3:
            await self.node._send(
                op="filters", **self.filters, guildId=str(self.guild_id)
            )
        else:
            await self.node.update_player(
                guild_id=self.guild_id, data={"filters": self.filters}
            )
