...
class VolumeInteraction(disnake.ui.View):
        ...
        for l in [5, 20, 40, 60, 80, 100, 120, 150]:
            ...
            opts.append(
                disnake.SelectOption(
                    label=f"{l}%", value=f"vol_{l}", description=description
                )
            )
    ...
    async def callback(self, interaction: disnake.MessageInteraction):
        await interaction.response.edit_message(
            content=f"Volume alterado!", embed=None, view=None
        )
...
class QueueInteraction(disnake.ui.View):
    ...
    def update_pages(self, reset_page=True):
        ...
        self.track_pages = list(
            disnake.utils.as_chunks(
                player.queue or player.queue_autoplay, max_size=self.max_items
            )
        )
    ...
    def update_components(self):
        ...
        play = disnake.ui.Button(
            emoji="▶️",
            label="Tocar",
            style=disnake.ButtonStyle.grey,
            custom_id="queue_skip",
        )
        play.callback = self.invoke_command
        self.add_item(play)

        move = disnake.ui.Button(
            emoji="↪️",
            label="Mover",
            style=disnake.ButtonStyle.grey,
            custom_id="queue_move",
        )
        move.callback = self.move_callback
        self.add_item(move)

        rotate_q = disnake.ui.Button(
            emoji="🔃",
            label="Rotacionar Fila",
            style=disnake.ButtonStyle.grey,
            custom_id="queue_rotate",
        )
        rotate_q.callback = self.invoke_command
        self.add_item(rotate_q)

        update_q = disnake.ui.Button(
            emoji="🔄", label="Recarregar", style=disnake.ButtonStyle.grey
        )
    ...
    async def move_callback(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Mover música selecionada",
                custom_id="queue_move_modal",
                components=[
        ...
        try:
            if inter.data.custom_id == "queue_move_modal":
                ...
                if not inter.text_values["queue_move_position"].isdigit():
                    await inter.send(
                        "Você deve usar um número válido...", ephemeral=True
                    )
                    return
                ...
                if move_func:
                    move_func(
                        (int(inter.text_values["queue_move_position"]) or 1) - 1,
                        self.current_track,
                    )
                    player.update = True
                ...
                await inter.send(
                    f"Método ainda não implementado: {inter.data.custom_id}",
                    ephemeral=True,
                )
    ...
    def update_embed(self):
        ...
        self.embed.title = (
            f"**Músicas da fila [Página: {self.current_page+1} / {self.max_page+1}]**"
        )
        ...
        for n, t in enumerate(self.track_pages[self.current_page]):
            ...
            if self.current_track == t:
                ...
                txt += (
                    f"`┌ {index})` [`{fix_characters(t.title, limit=45)}`]({t.uri})\n"
                    f"`└ ⏲️ {duration}`"
                    + (f" - `Repetições: {t.track_loops}`" if t.track_loops else "")
                    + f" **|** "
                    + (
            ...
            opts.append(
                disnake.SelectOption(
                    label=f"{index}. {t.author}"[:25],
                    description=f"[{duration}] | {t.title}"[:50],
                    value=f"queue_select_{t.unique_id}",
                    default=t == self.current_track,
                )
            )
    ...
    async def track_select_callback(self, interaction: disnake.MessageInteraction):
        ...
        if not track:
            await interaction.send(
                f'Música com id "{track_id}" não encontrada na fila do player...',
                ephemeral=True,
            )
            return
    ...
    async def invoke_command(self, interaction: disnake.MessageInteraction):
        ...
        try:
            ...
            if interaction.data.custom_id == "queue_skip":
                if (
                    player.current
                    and player.current.unique_id == self.current_track.unique_id
                ):
                    await check_cmd(self.bot.get_slash_command("seek"), interaction)
                    await player.seek(0)
                    player.set_command_log(
                        emoji="⏪",
                        text=f"{interaction.author.mention} retrocedeu da música para: `0:00`",
                    )
                    player.update = True
                    ...
                    kwargs = {
                        "query": f"{self.current_track.title} || ID > {self.current_track.unique_id}"
                    }
                ...
                await interaction.send(
                    f"Comando não implementado: {interaction.data.custom_id}",
                    ephemeral=True,
                )
                return
    ...
    async def stop_interaction(self, interaction: disnake.MessageInteraction):
        ...
        await interaction.response.edit_message(
            content="Queue fechada", embed=None, view=None
        )
        self.stop()
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction):
        ...
        if interaction.author != self.user:
            await interaction.send(
                f"Apenas o membro {self.user.mention} pode interagir aqui.",
                ephemeral=True,
            )
            return
        ...
        try:
            ...
            await interaction.response.edit_message(
                content="O player foi finalizado...", embed=None, view=None
            )
            self.stop()
            return
...
class EmbedPaginatorInteraction(disnake.ui.View):
    ...
    async def interaction_check(self, interaction: disnake.MessageInteraction) -> bool:
        ...
        await interaction.send(
            f"Apenas {self.user.mention} pode interagir aqui.", ephemeral=True
        )
...
class SelectInteraction(disnake.ui.View):
    ...
    def load_components(self):
        ...
        select_menu = disnake.ui.Select(
            ...
            options=self.item_pages[self.current_page],
        )
...
class AskView(disnake.ui.View):
    ...
    def __init__(
        self, *, ctx: Union[commands.Context, disnake.Interaction], timeout=None
    ):
        super().__init__(timeout=timeout)
        self.selected = None
        self.ctx = ctx
...
async def process_idle_embed(bot: BotCore, guild: disnake.Guild, guild_data: dict):
    ...
    try:
        message = await channel.fetch_message(
            int(guild_data["player_controller"]["message_id"])
        )
    except:
        message = None
...
class FavModalImport(disnake.ui.Modal):
    ...
    def __init__(self, view):
        ...
        raise GenericError(
            f"Modo atual ainda não implementado: {self.view.mode} | {type(self.view.mode)}"
        )
    ...
    async def callback(self, inter: disnake.ModalInteraction, /) -> None:
        ...
        if self.view.mode == ViewMode.fav_manager:
            ...
            if (
                retry_after := self.view.bot.get_cog("Music")
                .fav_import_export_cd.get_bucket(inter)
                .update_rate_limit()
            ):
                if retry_after < 1:
                    retry_after = 1
                await inter.send(
                    "**Você deve aguardar {} para importar.**".format(
                        time_format(int(retry_after) * 1000, use_names=True)
                    ),
                    ephemeral=True,
                )
                return
            ...
            for name, url in json_data.items():
                ...
                if len(url) > (
                    max_url_chars := self.view.bot.config["USER_FAV_MAX_URL_LENGTH"]
                ):
                    await inter.send(
                ...
                if not isinstance(url, str) or not URL_REG.match(url):
                    await inter.send(
                        f"O seu arquivo contém link inválido: ```ldif\n{url}```",
                        ephemeral=True,
                    )
                    return
            ...
            self.view.data = await self.view.bot.get_global_data(
                inter.author.id, db_name=DBModel.users
            )

            for name in json_data.keys():
                if len(name) > (
                    max_name_chars := self.view.bot.config["USER_FAV_MAX_NAME_LENGTH"]
                ):
            ...
            if self.view.bot.config["MAX_USER_FAVS"] > 0 and not (
                await self.view.bot.is_owner(inter.author)
            ):

                if (json_size := len(json_data)) > self.view.bot.config[
                    "MAX_USER_FAVS"
                ]:
                    await inter.edit_original_message(
                ...
                if (
                    json_size + (user_favs := len(self.view.data["fav_links"]))
                ) > self.view.bot.config["MAX_USER_FAVS"]:
                    await inter.edit_original_message(
            ...
            await self.view.bot.update_global_data(
                inter.author.id, self.view.data, db_name=DBModel.users
            )
            ...
            self.view.guild_data = await self.view.bot.get_data(
                inter.guild_id, db_name=DBModel.guilds
            )

            if not self.view.guild_data["player_controller"][
                "channel"
            ] or not self.view.bot.get_channel(
                int(self.view.guild_data["player_controller"]["channel"])
            ):
                await inter.edit_original_message(
                    "**Não há player configurado no servidor! Use o comando /setup**"
                )
                return
            ...
            if (
                ...
                + (
                    user_favs := len(
                        self.view.guild_data["player_controller"]["fav_links"]
                    )
                )
            ) > 25:
                await inter.edit_original_message(
            ...
            await self.view.bot.update_data(
                inter.guild_id, self.view.guild_data, db_name=DBModel.guilds
            )

            guild = self.view.bot.get_guild(inter.guild_id)

            await inter.edit_original_message(
                content="**Links fixos do servidor foram importados com sucesso!**"
            )
            ...
            await process_idle_embed(
                self.view.bot, guild, guild_data=self.view.guild_data
            )
            ...
            if self.view.bot.config["MAX_USER_INTEGRATIONS"] > 0 and not (
                await self.view.bot.is_owner(inter.author)
            ):
                ...
                if (json_size := len(json_data)) > self.view.bot.config[
                    ...
                    await inter.edit_original_message(
                        f"A quantidade de itens no seu arquivo de integrações excede "
                ...
                if (
                    json_size
                    + (user_integrations := len(self.view.data["integration_links"]))
                ) > self.view.bot.config["MAX_USER_INTEGRATIONS"]:
                    await inter.edit_original_message(
            ...
            raise GenericError(
                f"**Modo ainda não implementado: {self.view.mode} | {type(self.view.mode)}**"
            )

        if not isinstance(self.view.ctx, CustomContext):
            await self.view.ctx.edit_original_message(
...
class FavModalAdd(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        if self.view.mode == ViewMode.fav_manager:
            ...
            try:
                if name != self.name:
                    del self.view.guild_data["player_controller"]["fav_links"][
                        self.name
                    ]
            except KeyError:
            ...
            self.view.guild_data["player_controller"]["fav_links"][name] = {
                "url": valid_url,
            ...
            await inter.edit_original_message(
                embed=disnake.Embed(
                    description="**Link adicionado/atualizado com sucesso nos fixos do player!\n"
                    "Membros podem usá-lo diretamente no player-controller quando não estiver em uso.**",
                    color=self.view.bot.get_color(guild.me),
                ),
                view=None,
            )
            ...
            if (
                len(self.view.data["integration_links"])
                >= self.view.bot.config["MAX_USER_FAVS"]
            ):
                await inter.edit_original_message(
            ...
            if matches := spotify_regex_w_user.match(url):
                ...
                data = {
                    "title": f"[SP]: {result['display_name'][:90]}",
                ...
                try:
                    info = await loop.run_in_executor(
                        None,
                        lambda: self.view.bot.pool.ytdl.extract_info(
                            base_url, download=False
                        ),
                    )
                except Exception as e:
                    traceback.print_exc()
                    await inter.edit_original_message(
                        f"**Ocorreu um erro ao obter informação da url:** ```py\n{repr(e)}```"
                    )
                    return
        ...
        if not isinstance(self.view.ctx, CustomContext):
            await self.view.ctx.edit_original_message(
                content=self.view.build_txt(), view=self.view
            )
        elif self.view.message:
...
class FavMenuView(disnake.ui.View):
    ...
    def update_components(self):
        ...
        if not self.light_mode:
            ...
            mode_select = disnake.ui.Select(
                options=[
                    disnake.SelectOption(
                        label="Gerenciador de Favoritos",
                        value=f"fav_view_mode_{ViewMode.fav_manager}",
                        emoji="⭐",
            ...
            mode_select.append_option(
                disnake.SelectOption(
                    label="Gerenciador de Integrações",
                    value=f"fav_view_mode_{ViewMode.integrations_manager}",
                    emoji="💠",
            ...
            if self.guild and (
                self.ctx.author.guild_permissions.manage_guild or self.is_owner
            ):
                mode_select.options.insert(
                    1,
                    disnake.SelectOption(
                        label="Gerenciador de Playlists do Servidor",
                        value=f"fav_view_mode_{ViewMode.guild_fav_manager}",
                        emoji="📌",
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            if self.data["fav_links"]:
                opts = []
                for k, v in list(self.data["fav_links"].items())[
                    :25
                ]:  # TODO: Lidar depois com os dados existentes que excedem a quantidade permitida
                    emoji, platform = music_source_emoji_url(v)
                    opts.append(
                        disnake.SelectOption(label=k, emoji=emoji, description=platform)
                    )
            ...
            for b in sorted(
                self.bot.pool.get_guild_bots(self.guild.id), key=lambda b: b.identifier
            ):
                if b.bot_ready and b.user in self.guild.members:
                    bots_in_guild.append(
                        disnake.SelectOption(
                            emoji="🎶",
                            label=f"Bot: {b.user.display_name}"[:25],
                            value=f"bot_select_{b.user.id}",
                            description=f"ID: {b.user.id}",
                            default=b == self.bot,
                        )
                    )

            if bots_in_guild:
                bot_select = disnake.ui.Select(
                    options=bots_in_guild, min_values=1, max_values=1
                )
                bot_select.callback = self.bot_select
                self.add_item(bot_select)

            if self.guild_data["player_controller"]["fav_links"]:
                fav_select = disnake.ui.Select(
                    options=[
                        disnake.SelectOption(
                            label=k,
            ...
            if self.data["integration_links"]:
                opts = []
                for k, v in list(self.data["integration_links"].items())[
                    :25
                ]:  # TODO: Lidar depois com os dados existentes que excedem a quantidade permitida
                    ...
                    opts.append(
                        disnake.SelectOption(
                            label=k[5:], emoji=emoji, description=platform, value=k
                        )
                    )
                integration_select = disnake.ui.Select(
        ...
        add_button = disnake.ui.Button(
            label="Adicionar", emoji="<:add_music:588172015760965654>"
        )
        ...
        if self.mode == ViewMode.fav_manager:
            edit_button = disnake.ui.Button(
                label="Editar", emoji="✍️", disabled=not self.data["fav_links"]
            )
            edit_button.callback = self.edit_callback
            self.add_item(edit_button)

            remove_button = disnake.ui.Button(
                label="Remover", emoji="♻️", disabled=not self.data["fav_links"]
            )
            ...
            clear_button = disnake.ui.Button(
                ...
                emoji="🚮",
                disabled=not self.data["fav_links"],
            )
            clear_button.callback = self.clear_callback
            self.add_item(clear_button)

            export_button = disnake.ui.Button(
                label="Exportar", emoji="📤", disabled=not self.data["fav_links"]
            )
            ...
            edit_button = disnake.ui.Button(
                label="Editar",
                emoji="✍️",
                disabled=not self.guild_data["player_controller"]["fav_links"],
            )
            edit_button.callback = self.edit_callback
            self.add_item(edit_button)
            ...
            remove_button = disnake.ui.Button(
                label="Remover", emoji="♻️", disabled=not self.data["integration_links"]
            )
            remove_button.callback = self.remove_callback
            self.add_item(remove_button)

            clear_button = disnake.ui.Button(
                label="Limpar Integrações",
                emoji="🚮",
                disabled=not self.data["integration_links"],
            )
            clear_button.callback = self.clear_callback
            self.add_item(clear_button)
        ...
        if self.mode == ViewMode.fav_manager:
            if self.data["fav_links"] and not self.light_mode:
                play_button = disnake.ui.Button(
                    label="Tocar o favorito selecionado",
                    emoji="▶",
                    custom_id="favmanager_play_button",
                )
            ...
            if self.data["integration_links"]:
                play_button = disnake.ui.Button(
                    label="Tocar uma playlist da integração selecionada",
                    emoji="▶",
                    custom_id="favmanager_play_button",
                )
                play_button.callback = self.play_callback
    ...
    async def auto_update(self):
        ...
        while True:
            ...
            user, data, url = await self.bot.wait_for(
                "fav_add", check=lambda user, data, url: user.id == self.ctx.author.id
            )
            ...
            if not isinstance(self.ctx, CustomContext):
                await self.ctx.edit_original_message(
                    content=self.build_txt(), view=self
                )
            elif self.message:
    ...
    def build_txt(self):
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            if not self.data["fav_links"]:
                txt += (
                    "Você não possui favoritos (clique no botão de adicionar abaixo).\n"
                )
                ...
                txt += "\n".join(
                    f"> {format_fav(n+1, d)}"
                    for n, d in enumerate(islice(self.data["fav_links"].items(), 25))
                )
            ...
            if not self.guild_data["player_controller"]["fav_links"]:
                ...
                txt += (
                    f"**Links atuais no bot {self.bot.user.mention}:**\n"
                    + "\n".join(
                        f"> {format_gfav(n+1, d)}"
                        for n, d in enumerate(
                            islice(
                                self.guild_data["player_controller"][
                                    "fav_links"
                                ].items(),
                                25,
                            )
            ...
            if not self.data["integration_links"]:
                ...
                txt += f"### Suas integrações atuais:\n" + "\n".join(
                    f"> {format_itg(self.bot, n+1, d)}"
                    for n, d in enumerate(
                        islice(self.data["integration_links"].items(), 25)
                    )
                )
            ...
            raise GenericError(
                f"**Modo não implementado:** {self.mode} | {type(self.mode)}"
            )
    ...
    async def edit_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            try:
                ...
                await inter.send(
                    f"**Não há favorito com o nome:** {self.current}", ephemeral=True
                )
            ...
            if not guild:
                await inter.send(
                    "Você não pode executar essa ação fora de um servidor.",
                    ephemeral=True,
                )
                return
            try:
                await inter.response.send_modal(
                    FavModalAdd(
                        bot=self.bot,
                        name=self.current,
                        url=self.data["player_controller"]["fav_links"][self.current][
                            "url"
                        ],
                        description=self.data["player_controller"]["fav_links"][
                            self.current
                        ]["description"],
                        view=self,
                    )
                ...
                await inter.send(
                    f"**Não há favorito com nome:** {self.current}", ephemeral=True
                )
    ...
    async def remove_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.guild_fav_manager:
            ...
            if not self.guild_data:
                self.guild_data = await self.bot.get_data(
                    inter.guild_id, db_name=DBModel.guilds
                )

        else:
            self.data = await self.bot.get_global_data(
                inter.author.id, db_name=DBModel.users
            )
        ...
        if self.mode == ViewMode.fav_manager:
            try:
                ...
                await inter.edit_original_message(
                    f"**Não há favorito na lista com o nome:** {self.current}"
                )
                return

            await self.bot.update_global_data(
                inter.author.id, self.data, db_name=DBModel.users
            )
            ...
            try:
                ...
                try:
                    await process_idle_embed(
                        self.bot, guild, guild_data=self.guild_data
                    )
                except Exception:
                    traceback.print_exc()
            ...
            await self.bot.update_data(
                inter.guild_id, self.guild_data, db_name=DBModel.guilds
            )
            ...
            try:
                ...
                await inter.send(
                    f"**Não há integração na lista com o nome:** {self.current}",
                    ephemeral=True,
                )
                return
    ...
    async def clear_callback(self, inter: disnake.MessageInteraction):
        ...
        if self.mode == ViewMode.fav_manager:
            ...
            await inter.send(
                "### Seus favoritos foram excluídos com sucesso!\n"
                ...
                ephemeral=True,
                file=disnake.File(fp, filename="favs.json"),
            )
            ...
            if not self.guild_data["player_controller"]["fav_links"]:
                await inter.send(
                    "**Não há links favoritos no servidor.**", ephemeral=True
                )
                return
            ...
            fp = BytesIO(
                bytes(
                    json.dumps(
                        self.guild_data["player_controller"]["fav_links"], indent=4
                    ),
                    "utf-8",
                )
            )
            ...
            await inter.send(
                "### Os links de favoritos do server foram excluídos com sucesso!\n"
                "`um arquivo de backup foi gerado e caso queira reverter essa exclusão, copie o "
                'conteúdo do arquivo e clique no botão "importar" e cole o conteudo no campo indicado.`',
                ephemeral=True,
                file=disnake.File(fp, filename="guild_favs.json"),
            )
            ...
            if not self.data["integration_links"]:
                await inter.response.edit_message(
                    content="**Você não possui integrações salvas!**", view=None
                )
                return

            fp = BytesIO(
                bytes(json.dumps(self.data["integration_links"], indent=4), "utf-8")
            )
            ...
            await inter.send(
                "### Suas integrações foram excluídas com sucesso!\n"
                "`um arquivo de backup foi gerado e caso queira reverter essa exclusão, copie o "
                'conteúdo do arquivo e clique no botão "importar" e cole o conteudo no campo indicado.`',
                ephemeral=True,
                file=disnake.File(fp, filename="integrations.json"),
            )
    ...
    async def play_callback(self, inter: disnake.MessageInteraction):
        ...
        await self.bot.get_cog("Music").player_controller(
            inter,
            PlayerControls.enqueue_fav,
            query=(
                f"> itg: {self.current}"
                if self.mode == ViewMode.integrations_manager
                else f"> fav: {self.current}"
            ),
        )
    ...
    async def export_callback(self, inter: disnake.MessageInteraction):
        ...
        if retry_after := cog.fav_import_export_cd.get_bucket(
            inter
        ).update_rate_limit():
            if retry_after < 1:
                retry_after = 1
            await inter.send(
                "**Você deve aguardar {} para exportar.**".format(
                    time_format(int(retry_after) * 1000, use_names=True)
                ),
                ephemeral=True,
            )
        ...
        try:
            cmd = (
                f"</{cog.fav_manager.name}:"
                + str(
                    self.bot.get_global_command_named(
                        cog.fav_manager.name,
        ...
        if self.mode == ViewMode.fav_manager:
            if not self.data["fav_links"]:
                await inter.send(
                    f"**Você não possui links favoritos..\n"
            ...
            await inter.send(
                ...
                file=disnake.File(fp=fp, filename="favoritos.json"),
                ephemeral=True,
            )
            ...
            if not self.guild_data["player_controller"]["fav_links"]:
                await inter.edit_original_message(
                    content=f"**Não há músicas/playlists fixadas no servidor..\n"
                    f"Você pode adicionar usando o comando: {cmd}**"
                )
            ...
            await inter.send(
                embed=embed,
                file=disnake.File(fp=fp, filename="guild_favs.json"),
                ephemeral=True,
            )
            ...
            if not self.data["integration_links"]:
                await inter.edit_original_message(
                    f"**Você não possui integrações adicionadas...\n"
                    f"Você pode adicionar usando o comando: {cmd}**"
                )
                return
            ...
            await inter.send(
                ...
                file=disnake.File(fp=fp, filename="integrations.json"),
                ephemeral=True,
            )
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        await inter.send(
            f"Apenas o membro {self.ctx.author.mention} pode interagir nessa mensagem.",
            ephemeral=True,
        )
...
base_skin = {
    ...
    "embeds": [
        ...
        },
        {
            ...
            "color": "{guild.color}",
            "footer": {"text": "Músicas na lista: {player.queue.size}"},
        },
    ],
}
...
class SkinSettingsButton(disnake.ui.View):
    ...
    def update_components(self):
        ...
        select_mode = disnake.ui.Select(
            ...
            options=[
                disnake.SelectOption(
                    label="Modo Normal",
                    description="Aplicar skin ao modo normal do player",
                    value="custom_skins",
                    default=self.mode == "custom_skins",
                ),
                disnake.SelectOption(
                    label="Song-Request",
                    description="Aplicar skin no modo song-request do player",
                    value="custom_skins_static",
                    default=self.mode == "custom_skins_static",
                ),
            ],
        )
        ...
        if self.mode == "custom_skins":
            controller_btn = disnake.ui.Button(
                emoji="💠",
                label=(
                    "Activate Player-Controller"
                    if not self.controller_enabled
                    else "Disable Player-Controller"
                ),
            )
    ...
    async def interaction_check(self, inter: disnake.MessageInteraction) -> bool:
        ...
        if inter.user.id != self.user.id:
            await inter.send(
                f"Apenas o membro {self.user.mention} pode usar es botões da mensagem.",
                ephemeral=True,
            )
            return False
...
class ViewModal(disnake.ui.Modal):
    ...
    def __init__(
        self,
        view: Union[SkinEditorMenu, QueueInteraction],
        title: str,
        components: List[disnake.TextInput],
        custom_id: str,
    ):
        self.view = view
...
class SetStageTitle(disnake.ui.View):
    ...
    placeholder_text = (
        ...
        "[34;1m{track.timestamp}[0m -> Duração da música em contagem regressiva (apenas em voice channel).\n"
        "[34;1m{track.source}[0m -> Origem/Fonte da música (Youtube/Spotify/Soundcloud etc)\n"
        "[34;1m{track.emoji}[0m -> Emoji da fonte de música (apenas em voice channel).\n"
        ...
        "`Nota: No voice channel você pode usar custom emojis na mensagem do status (incluindo emoji de servers que eu não estou e de servers que você não está).`"
    )

    def __init__(
        self,
        ctx: Union[CustomContext, disnake.Interaction],
        bot: BotCore,
        guild: disnake.Guild,
        data: dict,
    ):
        ...
        if self.data["voice_channel_status"]:
            setstatus_btn = disnake.ui.Button(
                label="Usar modelo atual",
                emoji="🎶",
                style=disnake.ButtonStyle.grey,
        ...
        setstatus_modal_btn = disnake.ui.Button(
            label="Activate/Disable status", emoji="🔊", style=disnake.ButtonStyle.grey
        )
        setstatus_modal_btn.callback = self.set_status_modal
        self.add_item(setstatus_modal_btn)

        setstatus_perm_btn = disnake.ui.Button(
            label="Activate/Disable status (permanente)",
            emoji="💾",
            style=disnake.ButtonStyle.grey,
        )
    ...
    async def setstatus_callback(self, interaction: disnake.MessageInteraction):
        await self.modal_handler(
            inter=interaction,
            values={"status_voice_value": self.data["voice_channel_status"]},
        )
            ...
            ViewModal(
                components=[
                    disnake.ui.TextInput(
                        ...
                        placeholder="Pra disable deixe vazio",
                        max_length=496,
                        required=False,
                    ),
                ],
    ...
    def build_embeds(self):
        ...
        embeds.append(
            disnake.Embed(
                description="### Definir status automático no voice channel ou palco\n"
        ...
        if self.data["voice_channel_status"]:
            embeds.append(
                disnake.Embed(
                    title="**Modelo permanente atual:**",
                    description=self.data["voice_channel_status"],
                    color=self.bot.get_color(self.guild.me),
                )
            )
    ...
    async def modal_handler(self, inter: disnake.ModalInteraction, values: dict = None):
        ...
        values["status_voice_value"] = (
            values["status_voice_value"].replace("\n", " ").strip()
        )
        ...
        if values["status_voice_value"] and not any(
            ...
            await inter.send(
                "**Você deve usar pelo menos um placeholder válido...**", ephemeral=True
            )
            return
        ...
        if inter.data.custom_id == "status_voice_channel_perm":
            ...
            if self.data["voice_channel_status"] == values["status_voice_value"]:
                await inter.send(
                    "**O status permanente atual é o mesmo do informado...**",
                    ephemeral=True,
                )
                return
            ...
            if not inter.author.guild_permissions.manage_guild:
                await inter.send(
                    "**Você não possui a permissão de gerenciar servidor para alterar o status do voice channel**",
                    ephemeral=True,
                )
                return
            ...
            await self.bot.update_global_data(
                inter.guild_id, self.data, db_name=DBModel.guilds
            )
            ...
            for b in self.bot.pool.get_guild_bots(inter.guild_id):
                ...
                p.set_command_log(
                    text=f"{inter.author.mention} "
                    + ("ativou" if values["status_voice_value"] else "desativou")
                    + " o status automático",
                    emoji="📢",
                )
                p.update = True
            ...
            await inter.edit_original_message(
                "**Status permanente foi "
                + ("salvo" if values["status_voice_value"] else "desativado")
                + " com sucesso!**"
            )
            ...
            if not player:
                await inter.send(
                    "**Não estou tocando música em um voice channel/palco...**",
                    ephemeral=True,
                )
                return
            ...
            await inter.edit_original_message(
                "**Status definido com sucesso!**"
                if values["status_voice_value"]
                else "**Status desativado com sucesso!**"
            )

        else:
            await inter.send(
                f"Não implementado: {inter.data.custom_id}", ephemeral=True
            )
            return
...
class SkinEditorMenu(disnake.ui.View):
    ...
    def __init__(
        self,
        ctx: Union[CustomContext, disnake.AppCmdInter],
        bot: BotCore,
        guild: disnake.Guild,
        global_data: dict,
    ):
    ...
    def update_components(self):
        ...
        if self.mode == "select":
            ...
            skins_opts = [
                disnake.SelectOption(
                    emoji="💠",
                    label=f"Modo normal: {s.replace('> cs: ', '', 1)}",
                    value=s,
                )
                for s in add_skin_prefix(self.global_data["custom_skins"])
            ]
            add_skin_prefix = lambda d: [f"> css: {i}" for i in d.keys()]
            static_skins_opts = [
                disnake.SelectOption(
                    emoji="💠",
                    label=f"Song-Request: {s.replace('> css: ', '', 1)}",
                    value=s,
                )
                for s in add_skin_prefix(self.global_data["custom_skins_static"])
            ]

            has_skins = False

            if skins_opts:
                skin_select = disnake.ui.Select(
                    min_values=1,
                    max_values=1,
                    options=skins_opts,
            ...
            if static_skins_opts:
                static_skin_select = disnake.ui.Select(
                    min_values=1,
                    max_values=1,
                    options=static_skins_opts,
            ...
            if not has_skins:
                self.message_data = {
                    "embeds": [
                        {
                            "description": "**Não há skins salvas...\nClique no botão abaixo para criar uma nova skin/template.**",
                            "color": self.guild.me.color.value,
                        }
                    ]
                }
                new_skin_btn = disnake.ui.Button(
                    label="Adicionar nova skin",
                    custom_id="skin_editor_new_skin",
                    disabled=len(static_skins_opts) > 2 and len(skins_opts) > 2,
                )
                ...
                self.message_data = {
                    "embeds": [
                        {
                            "description": "**Selecione uma skin abaixo para editá-la ou crie uma nova usando um modelo base clicando no botão de adicionar abaixo.**",
                            "color": self.guild.me.color.value,
                        }
                    ]
                }
            ...
            if embeds := self.message_data.get("embeds"):
                ...
                select_embed = disnake.ui.Select(
                    ...
                    options=[
                        disnake.SelectOption(
                            label=f"Embed {n+1}",
                            value=f"skin_embed_{n}",
                            default=n == self.embed_index,
                        )
                ...
                if fields := embeds[self.embed_index].get("fields", []):
                    select_embed_field = disnake.ui.Select(
                        ...
                        options=[
                            disnake.SelectOption(
                                label=f"Field {n + 1}",
                                value=f"skin_embed_field_{n}",
                                default=n == self.embed_field_index,
                            )
                            for n, e in enumerate(fields)
                        ],
                    )
                ...
                if len(fields) < 25:
                    add_field_btn = disnake.ui.Button(
                        label="Adicionar Field", emoji="🔖"
                    )
                ...
                if fields:
                    ...
                    delete_field_btn = disnake.ui.Button(
                        label="Remover Field", emoji="🔖"
                    )
                ...
                set_author_footer_btn = disnake.ui.Button(
                    label="Embed Author + Footer", emoji="👤"
                )
                set_author_footer_btn.callback = self.set_author_footer
                self.add_item(set_author_footer_btn)

            edit_content_btn = disnake.ui.Button(
                label=(
                    "Adicionar" if not self.message_data.get("content") else "Editar"
                )
                + " Mensagem",
                emoji="💬",
            )
            ...
            add_embed_btn = disnake.ui.Button(
                label="Adicionar Embed", disabled=len(embeds) >= 8, emoji="📋"
            )
            add_embed_btn.callback = self.add_embed
            self.add_item(add_embed_btn)

            setup_queue_btn = disnake.ui.Button(
                label="Configurar placeholder da fila",
            ...
            save_disabled = (
                not embeds and len(self.message_data.get("content", "")) < 15
            )

            export_btn = disnake.ui.Button(
                label="Exportar Skin", emoji="📤", disabled=save_disabled
            )
            ...
            self.add_item(
                disnake.ui.Button(
                    label="Lista de placeholders",
                    emoji="<:help:947781412017279016>",
                    custom_id="skin_editor_placeholders",
                )
            )

            save_btn = disnake.ui.Button(
                label="Salvar Skin", emoji="💾", disabled=save_disabled
            )
            save_btn.callback = self.save
            self.add_item(save_btn)
    ...
    def build_embeds(self) -> dict:
        ...
        data = skin_converter(
            self.message_data, guild=self.guild, ctx=self.ctx, player=player
        )
    ...
    async def edit_content(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Editar/Adicionar conteúdo da mensagem",
                custom_id="skin_editor_message_content",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.long,
                        label="Mensagem:",
    ...
    async def add_embed(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                ...
                custom_id="skin_editor_add_embed",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
    ...
    async def edit_embed_button(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Edite os campos principais da embed",
                custom_id="skin_editor_edit_embed",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Título da embed:",
    ...
    async def add_field(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Adicionar field na embed",
                custom_id="skin_editor_add_field",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Nome:",
    ...
    async def edit_embed_field_button(self, inter: disnake.MessageInteraction):
        ...
        field = self.message_data["embeds"][self.embed_index]["fields"][
            self.embed_field_index
        ]

        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Edição dos campos principais da embed",
                custom_id="skin_editor_edit_field",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Nome do campo:",
    ...
    async def delete_embed_field_button(self, inter: disnake.MessageInteraction):
        self.ctx = inter
        del self.message_data["embeds"][self.embed_index]["fields"][
            self.embed_field_index
        ]
        self.embed_field_index = 0
        self.update_components()
    ...
    async def set_author_footer(self, inter: disnake.MessageInteraction):
        ...
        try:
            author_name = self.message_data["embeds"][self.embed_index]["author"][
                "name"
            ]
        except KeyError:
            author_name = ""
        ...
        try:
            author_icon_url = self.message_data["embeds"][self.embed_index]["author"][
                "icon_url"
            ]
        except KeyError:
            author_icon_url = ""
        ...
        try:
            footer_text = self.message_data["embeds"][self.embed_index]["footer"][
                "text"
            ]
        except KeyError:
            footer_text = ""
        ...
        try:
            footer_icon_url = self.message_data["embeds"][self.embed_index]["footer"][
                "icon_url"
            ]
        except KeyError:
            footer_icon_url = ""
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                custom_id="skin_editor_set_authorfooter",
                title="Adicionar/editar autor/footer",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
    ...
    async def setup_queue(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Placeholder da lista de músicas da fila",
                custom_id="skin_editor_setup_queue",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
    ...
    async def export(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_message(
            file=disnake.File(fp=fp, filename="skin.json"), ephemeral=True
        )
    ...
    async def import_(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Importar skin",
                custom_id="skin_editor_import_skin",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.long,
    ...
    async def save(self, inter: disnake.MessageInteraction):
        ...
        await inter.response.send_modal(
            ViewModal(
                view=self,
                title="Informe o nome da skin",
                custom_id="skin_editor_save",
                components=[
                    disnake.ui.TextInput(
                        style=disnake.TextInputStyle.short,
                        label="Nome:",
                        custom_id="skin_name",
                        value=self.skin_selected.replace("> css: ", "", 1).replace(
                            "> cs: ", "", 1
                        ),
                        max_length=15,
                        required=True,
    ...
    async def delete_skin(self, inter: disnake.MessageInteraction):
        ...
        self.global_data = await self.bot.get_global_data(
            id_=inter.guild_id, db_name=DBModel.guilds
        )
        ...
        if self.skin_selected.startswith("> cs:"):
            try:
                ...
                await inter.send(
                    f"**A skin {self.skin_selected[6:]} não existe mais na database...**",
                    ephemeral=True,
                )
                return
            ...
            try:
                ...
                await inter.send(
                    f"**A skin {self.skin_selected[7:]} não existe mais na database...**",
                    ephemeral=True,
                )
                return

        await self.bot.update_global_data(
            id_=inter.guild_id, data=self.global_data, db_name=DBModel.guilds
        )
    ...
    async def update_message(self, inter: disnake.MessageInteraction):
        ...
        try:
            ...
            traceback.print_exc()
            await inter.send(
                f"**Ocorreu um erro ao processar a mensagem:** ```py\n{repr(e)}```",
                ephemeral=True,
            )
    ...
    async def modal_handler(self, inter: disnake.ModalInteraction):
        ...
        if inter.custom_id == "skin_editor_message_content":
            ...
            e = (
                ...
                .set_thumbnail(inter.text_values["thumbnail_url"])
                .to_dict()
            )
            ...
            self.message_data["embeds"][self.embed_index]["title"] = inter.text_values[
                "skin_embed_title"
            ]
            self.message_data["embeds"][self.embed_index]["description"] = (
                inter.text_values["skin_embed_description"]
            )
            ...
            if not inter.text_values["image_url"]:
                ...
                self.message_data["embeds"][self.embed_index]["image"] = {
                    "url": inter.text_values["image_url"]
                }
            ...
            if not inter.text_values["thumbnail_url"]:
                ...
                self.message_data["embeds"][self.embed_index]["thumbnail"] = {
                    "url": inter.text_values["thumbnail_url"]
                }

            self.message_data["embeds"][self.embed_index]["color"] = inter.text_values[
                "skin_embed_color"
            ].strip("#")
            ...
            if not self.message_data["embeds"][self.embed_index].get("fields"):
                self.message_data["embeds"][self.embed_index]["fields"] = [
                    {
                        "name": inter.text_values["add_field_name"],
                        "value": inter.text_values["add_field_value"],
                    }
                ]
            else:
                self.message_data["embeds"][self.embed_index]["fields"].append(
                    {
                        "name": inter.text_values["add_field_name"],
                        "value": inter.text_values["add_field_value"],
                    }
            ...
            self.embed_field_index = (
                len(self.message_data["embeds"][self.embed_index]["fields"]) - 1
            )
            ...
            self.message_data["embeds"][self.embed_index]["fields"][
                ...
                "name": inter.text_values["edit_field_name"],
                "value": inter.text_values["edit_field_value"],
            }
            ...
            try:
                self.message_data["queue_max_entries"] = int(
                    inter.text_values["queue_max_entries"]
                )
            except TypeError:
                pass
            ...
            try:
                ...
                await inter.send(
                    f"**Ocorreu um erro ao processar sua skin:** ```py\n{repr(e)}```",
                    ephemeral=True,
                )
                return
            ...
            await inter.send(
                "**Selecione o modo de player que será aplicado na skin.**",
                view=view,
                ephemeral=True,
            )
            await view.wait()
            ...
            self.global_data = await self.bot.get_global_data(
                self.ctx.guild_id, db_name=DBModel.guilds
            )

            modal_skin_name = inter.text_values["skin_name"].strip()

            skin_name = self.skin_selected.replace("> css: ", "", 1).replace(
                "> cs: ", "", 1
            )
            ...
            self.global_data[view.mode][modal_skin_name] = b64encode(
                pickle.dumps(self.message_data)
            ).decode("utf-8")
            ...
            for bot in self.bot.pool.get_guild_bots(inter.guild_id):
                ...
                for n, s in global_data["custom_skins_static"].items():
                    if isinstance(s, str):
                        global_data["custom_skins_static"][n] = pickle.loads(
                            b64decode(s)
                        )
            ...
            await view.inter.edit_original_message(
                "**A skin foi salva/editada com sucesso!**\n"
                f"Você pode aplicá-la usando o comando {cmd} ou {guild_prefix}skin",
                view=None,
            )

            self.skin_selected = (
                "> cs: " if view.mode == "custom_skins" else "> css: "
            ) + modal_skin_name
...
class SelectBotVoice(disnake.ui.View):
    ...
    def build_interactions(self, freebots: List[BotCore] = None):
        ...
        if freebots:
            bot_select_opts.extend(
                [
                    disnake.SelectOption(
                        label=b.user.display_name, value=f"bot_voice_{b.user.id}"
                    )
                    for b in freebots
                ]
            )
            ...
            for b in self.inter.bot.pool.get_guild_bots(self.guild.id):
                ...
                if (
                    player
                    and self.inter.author.id not in player.last_channel.voice_states
                ):
                    continue

                bot_select_opts.append(
                    disnake.SelectOption(
                        label=b.user.display_name, value=f"bot_voice_{b.user.id}"
                    )
                )
        ...
        bot_select = disnake.ui.Select(
            min_values=0, max_values=1, options=bot_select_opts
        )
        bot_select.callback = self.bot_select_callback
        self.add_item(bot_select)

        refresh_btn = disnake.ui.Button(
            label="Atualizar lista", emoji="🔄", style=disnake.ButtonStyle.blurple
        )
    ...
    async def bot_select_callback(self, inter: disnake.MessageInteraction):
        ...
        try:
            bot = [
                b
                for b in self.inter.bot.pool.get_guild_bots(inter.guild_id)
                if b.bot_ready and b.user.id == bot_id
            ][0]
        except IndexError:
            await inter.send(
                f"<@{bot_id}> não está mais presente no servidor...", ephemeral=True
            )
        ...
        if not guild:
            await inter.send(
                f"{bot.user.mention} não está mais no servidor...", ephemeral=True
            )
            await self.update_message()
            return
        ...
        if player:
            ...
            if not vc:
                await inter.send(
                    f"{bot.user.mention} está com player ativo mas não está conectado em um voice channel...",
                    ephemeral=True,
                )
                await self.update_message()
                return
            ...
            if inter.author.id not in vc.voice_states:
                await inter.send(
                    f"{bot.user.mention} já está em uso no canal {vc.mention}",
                    ephemeral=True,
                )
                await self.update_message()
                return
